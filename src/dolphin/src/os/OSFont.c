...
inline BOOL IsSjisLeadByte(u8 letter)
{
    return (0x81 <= letter && letter <= 0x9F) || (0xE0 <= letter && letter <= 0xFC);
    // UNUSED FUNCTION
}

inline BOOL IsSjisTrailByte(u8 letter)
{
...
static int GetFontCode(u16 code)
{
    ...
    if (OSGetFontEncode() == OS_FONT_ENCODE_SJIS)
    {
        ...
        if (code >= 0x8140 && code < 0x879E)
        {
            ...
            return Zenkaku2Code[preCode + lastByte];
        }
    }
...
inline u32 ReadFont(void* img, u16 encode, void* fontInfo)
{
    SheetImage = fontInfo;
...
char* OSGetFontTexture(char* string, void** image, s32* x, s32* y, s32* width)
{
    ...
    if (OSGetFontEncode() == OS_FONT_ENCODE_SJIS)
    {
        if (((((u8)code >= 0x81) && ((u8)code <= 0x9F)) != FALSE ||
             (((u8)code >= 0xE0) && ((u8)code <= 0xFC))) != FALSE &&
            (*string != 0) != FALSE)
        {
