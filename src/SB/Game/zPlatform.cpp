    ...
    if (tmr >= 0.0f)
    {
        ...
        else
        {
            ...
            else
            {
                tmr -= time[0];
                lerp =
                    (0.5f * paddle->rotateSpeed * paddle->accelTime + tmr * paddle->rotateSpeed) /
                    absDelta;
            }
        }
    ...
    if (direction == 1)
    {
        if (!(plat->passet->paddle.paddleFlags & 0x1) ||
            (!(plat->passet->paddle.paddleFlags & 0x4) &&
             plat->ctr + 1 == plat->passet->paddle.countOrient))
        {
            stutter = 1;
        }
    }
    ...
    for (S32 i = 0; i < 12; i++)
    {
        ...
        if (tm > 0.0f)
        {
            ...
            if (fmrt->tmrs[i] < 0.0f)
            {
                ...
                F32 rdotu = -(pmat->right.x * pmat->up.x + pmat->right.y * pmat->up.y +
...
void zPlatform_Update(xEnt* entplat, xScene* sc, F32 dt)
{
    ...
    if (plat->subType == ePlatformTypeER)
    {
        ...
        if (plat->state == state || xEntERIsExtending(&plat->motion) ||
            xEntERIsExtended(&plat->motion) || xEntERIsRetracting(&plat->motion) ||
    ...
    else if (plat->subType == ePlatformTypeMech)
    {
        ...
        if (state != plat->state)
        {
            ...
            if (zMechIsStartingForth(plat, state) || zMechIsStartingBack(plat, state))
            {
                // do nothing
            }
            plat->state = state;
    ...
    else if (plat->subType == ePlatformTypeConvBelt)
    {
        ...
        if (plat == drv->odriver || plat == drv->driver)
        {
            ...
            xEnt* p = drv->driven;
            xVec3SMul(&p->frame->dpos, &xModelGetFrame(plat->model)->right,
                      s * plat->passet->cb.speed * dt);
    ...
    else if (plat->subType == ePlatformTypeTeeter)
    {
        ...
        if (plat != drv->odriver && plat != drv->driver)
        {
            ...
            if (ctilt != itilt)
            {
                ...
                xMat3x3Euler((xMat3x3*)plat->model->Mat, plat->frame->rot.axis.x,
                             plat->frame->rot.axis.y, plat->frame->rot.axis.z);
...
S32 zPlatformEventCB(xBase* from, xBase* to, U32 toEvent, const F32* toParam, xBase* toParamWidget)
{
    ...
    case eEventRun:
        ...
        {
            if (plat->subType == ePlatformTypeMech && globals.sceneCur->sceneID == 'PG12' &&
                 ...
                 plat->asset->modelInfoID == xStrHash("skatepark_flipper")))
            {
                ...
                {
                    xScrFXGlareAdd((xVec3*)&plat->model->Mat->pos, 0.5f * xurand() + 0.5f,
                                   0.5f * xurand() + 0.5f, 4.0f * xurand() + 3.0f, 1.0f, 0.7f, 0.5f,
                                   1.0f, NULL);
                    xScrFXGlareAdd((xVec3*)&plat->model->Mat->pos, 0.5f * xurand(), 0.8f * xurand(),
                                   3.0f + xurand(), xurand(), xurand(), xurand(), 1.0f, NULL);
                }
                xSndPlay3D(xStrHash("Check1"), 0.77f, 0.0f, 128, 0, (xVec3*)&plat->model->Mat->pos,
                           0.0f, SND_CAT_GAME, 0.0f);
                zRumbleStartDistance(globals.currentActivePad, 10.0f * xurand(), 48.0f,
