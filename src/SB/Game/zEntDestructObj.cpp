...
namespace
{
    static void SwapModel(zEntDestructObj* ent, xModelInstance* modelInst)
    {
        ...
        if (entModel != modelInst) {
            ...
            if (sfxDestroy != NULL) {
                xSndPlay3D(sfxDestroy->soundAssetID, (f32) sfxDestroy->volume, 0.0f, 0x80U, 0U, &sfxDestroy->pos, 0.0f, (sound_category) SND_CAT_GAME, 0.0f);
            }
        }
    }
}
...
void zEntDestructObj_Hit(zEntDestructObj* ent, unsigned int mask)
{
    if (ent->dasset->dflags & mask) {
        if (mask & 0x8000) {
            zEntPlayer_SNDPlayStreamRandom(0U, 0x10U, (_tagePlayerStreamSnd) 0x1A, (_tagePlayerStreamSnd) 0x1C, 0.1f);
            zEntPlayer_SNDPlayStreamRandom(0x10U, 0x23U, (_tagePlayerStreamSnd) 0x1A, (_tagePlayerStreamSnd) 0x1D, 0.1f);
            zEntPlayer_SNDPlayStreamRandom(0x24U, 0x64U, (_tagePlayerStreamSnd) 0x1A, (_tagePlayerStreamSnd) 0x1E, 0.1f);
        }
...
void zEntDestructObj_DestroyFX(zEntDestructObj* ent)
{
    ...
    if (sfxAsset != NULL) {
        xSndPlay3D(sfxAsset->soundAssetID, (F32) sfxAsset->volume, (F32) 0.0f, (U32) 0x80, (U32) 0x0, &sfxAsset->pos, (F32) 0.0f, (sound_category) 0, (F32) 0.0f);
    }
