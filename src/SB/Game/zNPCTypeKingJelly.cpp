...
void tweak_group::register_tweaks(bool init, xModelAssetParam* ap, U32 apsize, const char*)
{
    ...
    {
        move_radius = 13.0f;
        auto_tweak::load_param<F32, F32>(move_radius, 1.0, 0.0f, 20.f, ap, apsize, "move_radius");
    }
    if (init)
    {
        vel_decay = 0.8f;
    ...
    {
        music_fade = 0.5f;
        auto_tweak::load_param<F32, F32>(music_fade, 1.0f, 0.0f, 1.0f, ap, apsize, "music_fade");
    }
    if (init)
    {
    ...
    {
        spawn.cycles = 3;
        auto_tweak::load_param<S32, S32>(spawn.cycles, 1, 0, 100000, ap, apsize, "spawn.cycles");
    }
    if (init)
    {
        spawn.voffset = 3.0f;
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(spawn.spew.speed, 1.0f, 0.0f, 1000000000.0f, ap, apsize,
                                         "spawn.spew.speed");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(wave_ring.segment_length, 1.0f, 0.0099999998f, 10.0f, ap,
                                         apsize, "wave_ring.segment_length");
    }
    if (init)
    {
        wave_ring.particle_height = -0.3f;
        auto_tweak::load_param<F32, F32>(wave_ring.particle_height, 1.0f, -10.0f, 10.0f, ap, apsize,
                                         "wave_ring.particle_height");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(wave_ring.damage_height, 1.0f, -100.0f, 100.0f, ap, apsize,
                                         "wave_ring.damage_height");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(wave_ring.unit[0].radius_offset, 1.0f, -10.0f, 10.0f, ap,
                                         apsize, "wave_ring.unit[0].radius_offset");
    }
    if (init)
    {
        wave_ring.unit[0].height_offset = 1.0f;
        auto_tweak::load_param<F32, F32>(wave_ring.unit[0].height_offset, 1.0f, -10.0f, 10.0f, ap,
                                         apsize, "wave_ring.unit[0].height_offset");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(wave_ring.unit[1].radius_offset, 1.0f, -10.0f, 10.0f, ap,
                                         apsize, "wave_ring.unit[1].radius_offset");
    }
    if (init)
    {
        wave_ring.unit[1].height_offset = 0.0f;
        auto_tweak::load_param<F32, F32>(wave_ring.unit[1].height_offset, 1.0f, -10.0f, 10.0f, ap,
                                         apsize, "wave_ring.unit[1].height_offset");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(wave_ring.unit[2].radius_offset, 1.0f, -10.0f, 10.0f, ap,
                                         apsize, "wave_ring.unit[2].radius_offset");
    }
    if (init)
    {
        wave_ring.unit[2].height_offset = 0.0f;
        auto_tweak::load_param<F32, F32>(wave_ring.unit[2].height_offset, 1.0f, -10.0f, 10.0f, ap,
                                         apsize, "wave_ring.unit[2].height_offset");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(wave_ring.unit[3].radius_offset, 1.0f, -10.0f, 10.0f, ap,
                                         apsize, "wave_ring.unit[3].radius_offset");
    }
    if (init)
    {
        wave_ring.unit[3].height_offset = 0.0f;
        auto_tweak::load_param<F32, F32>(wave_ring.unit[3].height_offset, 1.0f, -10.0f, 10.0f, ap,
                                         apsize, "wave_ring.unit[3].height_offset");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(ambient_ring.min_height, 1.0f, -100.0f, 100.0f, ap, apsize,
                                         "ambient_ring.min_height");
    }
    if (init)
    {
        ambient_ring.max_height = 3.5f;
        auto_tweak::load_param<F32, F32>(ambient_ring.max_height, 1.0f, -100.0f, 100.0f, ap, apsize,
                                         "ambient_ring.max_height");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(ambient_ring.segment_length, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "ambient_ring.segment_length");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(ambient_ring.knock_back, 1.0f, 0.0f, 100.0f, ap, apsize,
                                         "ambient_ring.knock_back");
    }
    if (init)
    {
        ambient_ring.charge.radius = 4.5f;
        auto_tweak::load_param<F32, F32>(ambient_ring.charge.radius, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "ambient_ring.charge.radius");
    }
    if (init)
    {
        ambient_ring.charge.max_height = 8.0f;
        auto_tweak::load_param<F32, F32>(ambient_ring.charge.max_height, 1.0f, -100.0f, 100.0f, ap,
                                         apsize, "ambient_ring.charge.max_height");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(tentacle.move_degrees, 1.0f, 0.0f, 100000.0f, ap, apsize,
                                         "tentacle.move_degrees");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(tentacle.charge.thickness, 1.0f, 0.0f, 100.0f, ap, apsize,
                                         "tentacle.charge.thickness");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(tentacle.charge.move_degrees, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "tentacle.charge.move_degrees");
    }
    if (init)
    {
        thump.delay = 0.6f;
        auto_tweak::load_param<F32, F32>(thump.delay, 1.0f, 0.0f, 10.0f, ap, apsize, "thump.delay");
    }
    if (init)
    {
        thump.rings = 5;
    ...
    {
        thump.width = 2.0f;
        auto_tweak::load_param<F32, F32>(thump.width, 1.0f, 0.0f, 10.0f, ap, apsize, "thump.width");
    }
    if (init)
    {
        thump.vel = 10.0f;
        auto_tweak::load_param<F32, F32>(thump.vel, 1.0f, 0.0f, 10000.0f, ap, apsize, "thump.vel");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(sound[SOUND_AMBIENT_RING].delay, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_AMBIENT_RING].delay");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<S32, S32>(sound[SOUND_AMBIENT_RING].priority, 1, 0, 1000, ap, apsize,
                                         "sound[SOUND_AMBIENT_RING].priority");
    }
    if (init)
    {
        sound[SOUND_BIRTH].volume = 1.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_BIRTH].volume, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_BIRTH].volume");
    }
    if (init)
    {
        sound[SOUND_BIRTH].delay = 0.2f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_BIRTH].delay, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_BIRTH].delay");
    }
    if (init)
    {
        sound[SOUND_BIRTH].radius_inner = 20.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_BIRTH].radius_inner, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_BIRTH].radius_inner");
    }
    if (init)
    {
        sound[SOUND_BIRTH].radius_outer = 50.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_BIRTH].radius_outer, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_BIRTH].radius_outer");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(sound[SOUND_CHARGE].volume, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_CHARGE].volume");
    }
    if (init)
    {
        sound[SOUND_CHARGE].delay = 0.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_CHARGE].delay, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_CHARGE].delay");
    }
    if (init)
    {
        sound[SOUND_CHARGE].radius_inner = 10.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_CHARGE].radius_inner, 1.0f, 0.0f, 100000.0f,
                                         ap, apsize, "sound[SOUND_CHARGE].radius_inner");
    }
    if (init)
    {
        sound[SOUND_CHARGE].radius_outer = 40.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_CHARGE].radius_outer, 1.0f, 0.0f, 100000.0f,
                                         ap, apsize, "sound[SOUND_CHARGE].radius_outer");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(sound[SOUND_CHEER].volume, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_CHEER].volume");
    }
    if (init)
    {
        sound[SOUND_CHEER].delay = 0.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_CHEER].delay, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_CHEER].delay");
    }
    if (init)
    {
        sound[SOUND_CHEER].radius_inner = 20.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_CHEER].radius_inner, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_CHEER].radius_inner");
    }
    if (init)
    {
        sound[SOUND_CHEER].radius_outer = 50.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_CHEER].radius_outer, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_CHEER].radius_outer");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(sound[SOUND_GRUNT].volume, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_GRUNT].volume");
    }
    if (init)
    {
        sound[SOUND_GRUNT].delay = 0.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_GRUNT].delay, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_GRUNT].delay");
    }
    if (init)
    {
        sound[SOUND_GRUNT].radius_inner = 20.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_GRUNT].radius_inner, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_GRUNT].radius_inner");
    }
    if (init)
    {
        sound[SOUND_GRUNT].radius_outer = 50.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_GRUNT].radius_outer, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_GRUNT].radius_outer");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(sound[SOUND_LAND].volume, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_LAND].volume");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(sound[SOUND_LAND].radius_inner, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_LAND].radius_inner");
    }
    if (init)
    {
        sound[SOUND_LAND].radius_outer = 40.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_LAND].radius_outer, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_LAND].radius_outer");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(sound[SOUND_MOVE].volume, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_MOVE].volume");
    }
    if (init)
    {
        sound[SOUND_MOVE].radius_inner = 10.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_MOVE].radius_inner, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_MOVE].radius_inner");
    }
    if (init)
    {
        sound[SOUND_MOVE].radius_outer = 30.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_MOVE].radius_outer, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_MOVE].radius_outer");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(sound[SOUND_OSCILLATE].radius_inner, 1.0f, 0.0f, 100000.0f,
                                         ap, apsize, "sound[SOUND_OSCILLATE].radius_inner");
    }
    if (init)
    {
        sound[SOUND_OSCILLATE].radius_outer = 25.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_OSCILLATE].radius_outer, 1.0f, 0.0f, 100000.0f,
                                         ap, apsize, "sound[SOUND_OSCILLATE].radius_outer");
    }
    if (init)
    {
        sound[SOUND_OSCILLATE].priority = 0;
        auto_tweak::load_param<S32, S32>(sound[SOUND_OSCILLATE].priority, 1, 0, 1000, ap, apsize,
                                         "sound[SOUND_OSCILLATE].priority");
    }
    if (init)
    {
        sound[SOUND_RISE].volume = 1.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_RISE].volume, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_RISE].volume");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(sound[SOUND_RISE].radius_inner, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_RISE].radius_inner");
    }
    if (init)
    {
        sound[SOUND_RISE].radius_outer = 40.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_RISE].radius_outer, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_RISE].radius_outer");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(sound[SOUND_TAUNT].volume, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_TAUNT].volume");
    }
    if (init)
    {
        sound[SOUND_TAUNT].delay = 0.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_TAUNT].delay, 1.0f, 0.0f, 10.0f, ap, apsize,
                                         "sound[SOUND_TAUNT].delay");
    }
    if (init)
    {
        sound[SOUND_TAUNT].radius_inner = 20.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_TAUNT].radius_inner, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_TAUNT].radius_inner");
    }
    if (init)
    {
        sound[SOUND_TAUNT].radius_outer = 50.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_TAUNT].radius_outer, 1.0f, 0.0f, 100000.0f, ap,
                                         apsize, "sound[SOUND_TAUNT].radius_outer");
    }
    if (init)
    {
    ...
    if (init)
    {
        ...
        auto_tweak::load_param<F32, F32>(sound[SOUND_WAVE_RING].radius_inner, 1.0f, 0.0f, 100000.0f,
                                         ap, apsize, "sound[SOUND_WAVE_RING].radius_inner");
    }
    if (init)
    {
        sound[SOUND_WAVE_RING].radius_outer = 50.0f;
        auto_tweak::load_param<F32, F32>(sound[SOUND_WAVE_RING].radius_outer, 1.0f, 0.0f, 100000.0f,
                                         ap, apsize, "sound[SOUND_WAVE_RING].radius_outer");
    }
    if (init)
    {
        sound[SOUND_WAVE_RING].priority = 0;
        auto_tweak::load_param<S32, S32>(sound[SOUND_WAVE_RING].priority, 1, 0, 1000, ap, apsize,
                                         "sound[SOUND_WAVE_RING].priority");
    }
}
