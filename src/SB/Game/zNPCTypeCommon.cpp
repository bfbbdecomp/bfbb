...
S32 zNPCCommon::SndPlayFromSFX(xSFX* sfx, U32* sid_played)
{
    ...
    xsid = this->SndStart(aidToPlay, sprop, MIN(sfx->asset->outerRadius, this->cfg_npc->rad_sound));
...
U32 zNPCCommon::SndStart(U32 aid_toplay, NPCSndProp* sprop, F32 radius)
{
    ...
    else if (aid_toplay)
    {
        if (flg_snd & 0x2000)
        {
            xsid = xSndPlay3D(aid_toplay, 0.77f * vol, pvary, priority, xsndflags, this->Pos(),
                              2.0f, radius, SND_CAT_GAME, 0.0f);
        }
        else
        {
            xsid = xSndPlay3D(aid_toplay, 0.77f * vol, pvary, priority, xsndflags, (xEnt*)owner,
                              2.0f, radius, SND_CAT_GAME, 0.0f);
        }
    }
...
S32 zNPCCommon::LassoSetup()
{
    ...
    if (lass && lass->grabGuideModel && lass->grabGuideAnim && lass->holdGuideModel &&
        lass->holdGuideAnim)
    {
        // TODO , FIXME , Causes compiler error
...
S32 zNPCCommon::LassoGetAnims(xModelInstance* modgrab, xModelInstance* modhold)
{
    ...
    if (modhold->Anim && modhold->Anim)
    { // BUG: modgrab->Anim not checked
...
void zNPCCommon::AddBaseline(xPsyche* psy, xGoalProcessCallback eval_idle,
                             xGoalProcessCallback eval_patrol, xGoalProcessCallback eval_wander,
                             xGoalProcessCallback eval_waiting, xGoalProcessCallback eval_fidget)
{
    xGoal* goal;
...
void zNPCCommon::AddScripting(xPsyche* psy, xGoalProcessCallback eval_script,
                              xGoalProcessCallback eval_playanim, xGoalProcessCallback eval_attack,
                              xGoalProcessCallback eval_move, xGoalProcessCallback eval_follow,
                              xGoalProcessCallback eval_lead, xGoalProcessCallback eval_wait)
//NONMATCH("https://decomp.me/scratch/xuZpV")
{
    xGoal* goal;
...
void NPCC_BuildStandardAnimTran(xAnimTable* table, char** namelist, S32* ourAnims, S32 idx_dflt,
                                F32 blend) //NONMATCH("https://decomp.me/scratch/iKxSo")
{
    ...
    for (i = 0; ourAnims[i];)
    {
        ...
        xAnimTableNewTransition(table, names[ourAnims[i]], names[idx_dflt], NULL, NULL, 0, 0, 0.0f,
                                0.0f, 0, 0, blend, NULL);
        xAnimTableNewTransition(table, names[idx_dflt], names[ourAnims[i]], NULL, NULL, 0, 0, 0.0f,
                                0.0f, 0, 0, blend, NULL);
        i++;
    }
}
