...
static void zThrownCollide_DestructObj(zThrownStruct*, xEntCollis*, F32*, F32*);
static void zThrownCollide_Tiki(zThrownStruct*, xEntCollis*, F32*, F32*);
static void zThrownCollide_ThrowFreeze(zThrownStruct*, xEntCollis*, F32*, F32*);
static void zThrownCollide_ThrowFruit(zThrownStruct*, xEntCollis*, F32*, F32*);
static void zThrownCollide_BSandyHead(zThrownStruct*, xEntCollis*, F32*, F32*);
void zThrown_LaunchVel(xEnt*, xVec3*);
void zThrown_Remove(xEnt*);
static void zThrown_Update(xEnt*, xScene*, F32);
static void checkAgainstButtons(xEnt*);
static xBase* zThrown_ButtonIteratorCB(xBase*, zScene*, void*);
...
static ThrowableStats zThrowableModels[23] = {
    //  name                  collCB                       carry      launch     glowColor              shrapName                stackHeight
...
static FruitPatternEntry fruitPattern[] = {
    { 6.0f, 1.0f }, { 3.2f, 0.3f }, { 3.0f, 1.0f },  { 2.2f, 0.3f },  { 2.0f, 1.0f },
    { 1.2f, 0.3f }, { 1.0f, 1.0f }, { 0.0f, 0.15f }, { -1.0f, 0.0f },
};
...
static void zFruit_Update(xEnt* ent, xScene* sc, F32 dt)
{
    ...
    else if (ent->model->Scale.x == 1e-5f)
    {
        // Expired - check distance to player for cleanup
        F32 dist =
            xVec3Dist((xVec3*)&ent->model->Mat->pos, (xVec3*)&globals.player.ent.model->Mat->pos);
        if (dist < 1.4f)
        {
...
static void zThrown_Update(xEnt* ent, xScene* sc, F32 dt)
{
    ...
    {
        ent->bound.sph.center.y = -(0.25f * ent->bound.sph.r - ent->bound.sph.center.y);
    ...
    if (thrown->stackTgt != NULL)
    {
        ...
        if (frame->vel.x * (tgtMat->pos.x - frame->mat.pos.x) +
                frame->vel.z * (tgtMat->pos.z - frame->mat.pos.z) <=
            0.0f)
        {
            xVec3 velunit;
    ...
    if (0.05f < sws.dist && xSweptSphereToScene(&sws, globals.sceneCur, ent, 0x10))
    {
        F32 lerpdist = 0.25f * ent->bound.sph.r;
    ...
    if (thrown->stats->collCB != NULL &&
        (collis.stat_eidx > collis.stat_sidx || collis.dyn_eidx > collis.dyn_sidx ||
         collis.env_eidx > collis.env_sidx || collis.npc_eidx > collis.npc_sidx))
    {
        if (sSNDLandTimer == 0.0f)
        {
            xSndPlay3D(xStrHash("Tfruit_land4"), 1.0f, 0.0f, 0, 0, ent, 5.0f, 30.0f, 0, 0.0f);
        }
        sSNDLandTimer = 0.2f;
        ...
        else
        {
            ...
            for (S32 ci = 0; ci < 18; ci++)
            {
                if (collis.colls[ci].flags & k_HIT_IT)
                {
                    ...
                    F32 vy = thrown->vel.y;
                    F32 vz = thrown->vel.z;
                    dothdng = -collis.colls[ci].norm.x * vx + -collis.colls[ci].norm.y * vy +
                    ...
                    ent->frame->vel.x = vx - (1.0f + bounce) * nx - (1.0f - friction) * (vx - nx);
                    ent->frame->vel.y = vy - (1.0f + bounce) * ny - (1.0f - friction) * (vy - ny);
                    ent->frame->vel.z = vz - (1.0f + bounce) * nz - (1.0f - friction) * (vz - nz);
                    break;
                }
            }
        }
    ...
    case 1:
        if (thrown->stackTgt != NULL)
        {
            ...
            ent->bound.sph.center.y = 0.25f * oldbound.sph.r + ent->bound.sph.center.y;
        }
        break;
    case 2:
    {
    ...
    if (thrown->drv.driven == NULL)
    {
        if ((fcoll->flags & k_HIT_IT) == 0 || fcoll->optr == NULL ||
            (xEnt*)fcoll->optr != thrown->driveLastFloor || ((xEnt*)fcoll->optr)->frame == NULL ||
            thrown->vel.y > 0.0f)
        {
        ...
        else
        {
            ...
            {
                xEntDriveMount(&thrown->drv, (xEnt*)fcoll->optr, 0.1f, NULL);
            }
        }
    }
    else
    {
        if ((fcoll->flags & k_HIT_IT) == 0 || (xEnt*)fcoll->optr != thrown->drv.driven)
        {
            thrown->driveDebounce++;
...
S32 zThrown_LaunchPos(xEnt* ent, xVec3* pos, xVec3* dir)
{
    ...
    if (ent->baseType == eBaseTypeBoulder)
    {
        hpeak = stats->launch->throwSpeedY * tpeak + ent->bound.sph.center.y - ent->bound.sph.r -
                tpeak * (0.5f * grav * tpeak);
    }
    else
    {
        hpeak = stats->launch->throwSpeedY * tpeak + ent->model->Mat->pos.y -
                tpeak * (0.5f * grav * tpeak);
    }
        ...
        *thrown = zThrownList[zThrownCount - 1];
        ...
        if (ent->baseType == eBaseTypeBoulder)
        {
            ...
            if (thisCollCB == zThrownCollide_StoneTiki ||
                ...
                thisCollCB == zThrownCollide_BSandyHead || thisCollCB == zThrownCollide_ThrowFruit)
            {
                ent->chkby |= 0x10;
            }
        }
    ...
    if (collis->colls[0].flags & k_HIT_IT)
    {
        ...
        if (landEnt != NULL && landEnt->baseType == eBaseTypeStatic)
        {
            ...
            if (zThrown_IsFruit(landEnt, &stackHeight))
            {
                ...
                {
                    thrown->ent->model->Mat->pos.y = landEnt->model->Mat->pos.y + stackHeight;
                    *bounce = 0.0f;
                    *friction = 0.0f;
        ...
        {
            if ((F32)__fabs(collis->colls[0].norm.y) < 0.9659258f)
            {
                *friction = 1.0f;
...
static void zThrownCollide_ThrowFreeze(zThrownStruct* thrown, xEntCollis* collis, F32* bounce,
                                       F32* friction)
{
    sFruitIsFreezy = TRUE;
                                     ...
                                     F32* friction)
{
    ...
    *friction = 0.0f;
    ...
    if (collfound != 0)
    {
        if (collis->colls[collfound].dist < 0.707f)
        {
            *friction = 1.0f;
            *bounce = globals.player.carry.fruitWallBounce;
        }
...
static xBase* zThrown_ButtonIteratorCB(xBase* base, zScene* sc, void* data)
{
    xCollis collis;
    collis.flags = 0;
