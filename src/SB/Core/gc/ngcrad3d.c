...
HRAD3DIMAGE Open_RAD_3D_image(HBINK param_1, unsigned int param_2, unsigned int param_3, unsigned int param_4)
{
    HRAD3DIMAGE imgDat;
    int pi_p4;
    ...
    imgDat->g = GXGetTexBufferSize((unsigned short)param_2, (unsigned short)param_3, D3D_surface_type[param_4], 0, 0);
    imgDat->f = (void*)iFMVmalloc(imgDat->g);

    GXInitTexObj((volatile int)imgDat + 0x1c, imgDat->f, param_2 & 0xffff, param_3 & 0xffff, D3D_surface_type[param_4], 0, 0, 0);

    GXInitTexObjLOD(0.0f, 0.0f, 0.0f, (volatile int)imgDat + 0x1c, 1, 0, 0, 0, 0);
...
int Lock_RAD_3D_image(HRAD3DIMAGE rad_image, void* out_pixel_buffer, unsigned int* out_buffer_pitch, unsigned int* arg3)
{
    if (rad_image == 0)
    {
        return 0;
...
static void Submit_vertices(float param_1, float param_2, float param_3, float param_4, long param_5, long param_6, float param_7)
{
