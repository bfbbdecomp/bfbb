...
typedef u32(*type_18)(u32, xAnimActiveEffect*, xAnimSingle*, void*);
...
typedef s32(*type_99)(xBase*, xBase*, u32, f32*, xBase*);

typedef s8 type_1[20];
...
typedef f32 type_54[22];
typedef u8 type_55[2];
typedef u32 type_57[2];
typedef f32 type_58[22];
typedef u32 type_59[4];
typedef u8 type_60[2];
typedef u8 type_61[3];
typedef f32 type_62[6];
...
u8 delay_motive_update(widget& w, motive& m, f32 dt);
u8 shake_motive_update(motive& m);
u8 accelerate_motive_update(motive& m, f32 dt);
u8 linear_motive_update(motive& m, f32 dt);
void render_model(xModelInstance& m, render_context& rc);
xModelInstance* load_model(u32 id);
s32 cb_dispatch(xBase* from, xBase* to, u32 event, f32* argf, xBase* argw);
...
u8 is(u32 id);
u32 type();
void updater(widget* this, f32 dt);
void presetup(widget* this);
void destruct(widget* this);
void init_base(xBase& data, xBaseAsset& asset, u32 chunk_size);
...
void setup();
void init();
void for_each(u8 type, u32 size, functor_update f);
void for_each(u8 type, u32 size, type_73 f);
void for_each(u8 type, u32 size, functor_disable f);
