typedef struct NCINDRay;
typedef struct zCutsceneMgr;
typedef struct _tagLightningAdd;
typedef struct RpGeometry;
typedef struct xBase;
typedef struct xUpdateCullEnt;
typedef struct xShadowCache;
typedef struct xLightKit;
typedef struct xVec3;
typedef struct NCINEntry;
typedef struct xModelInstance;
typedef struct RwFrame;
typedef struct _class_0;
typedef enum en_LASSO_STATUS;
typedef struct RxPipeline;
typedef struct RwMatrixTag;
typedef struct RpSkin;
typedef struct xModelAssetParam;
typedef struct zFragProjectile;
typedef struct RxPacket;
typedef struct RpLight;
typedef struct RwCamera;
typedef struct RwRaster;
typedef struct zPlayerSettings;
typedef struct RxPipelineCluster;
typedef struct xMat4x3;
typedef struct RpWorld;
typedef struct RpAtomic;
typedef struct RxNodeDefinition;
typedef struct zFragProjectileAsset;
typedef struct xParEmitterAsset;
typedef struct NPCHazard;
typedef struct _tagxRumble;
typedef struct xModelTagWithNormal;
typedef struct xAnimTransitionList;
typedef struct zParEmitter;
typedef struct zLightning;
typedef struct RxPipelineRequiresCluster;
typedef struct zEnt;
typedef struct zNPCLassoInfo;
typedef enum RwFogType;
typedef struct RwTexCoords;
typedef struct RwResEntry;
typedef struct xEntFrame;
typedef struct xCoef3;
typedef struct NPCSndTrax;
typedef struct xAnimTable;
typedef struct _zPortal;
typedef struct zFragShockwaveAsset;
typedef struct iColor_tag;
typedef struct _tagEmitVolume;
typedef struct xParSysAsset;
typedef struct RpClump;
typedef struct NCINLamp;
typedef struct xCurveAsset;
typedef struct _zEnv;
typedef struct xEntOrbitData;
typedef struct RwRGBAReal;
typedef struct HAZTarTar;
typedef struct RwObjectHasFrame;
typedef enum en_dupowavmod;
typedef struct xAnimTransition;
typedef struct xClumpCollBSPTree;
typedef struct xGridBound;
typedef struct xParCmd;
typedef struct xPortalAsset;
typedef struct zFrag;
typedef enum zFragLocType;
typedef struct xEnt;
typedef struct xUpdateCullGroup;
typedef struct RpSector;
typedef struct RwLinkList;
typedef struct NCINStrk;
typedef struct xParSys;
typedef struct xJSPNodeInfo;
typedef struct HAZShroom;
typedef enum en_npchaz;
typedef struct RxPipelineNode;
typedef struct xAnimState;
typedef struct xEnvAsset;
typedef struct zPlatFMRunTime;
typedef struct xParGroup;
typedef struct xCutsceneZbufferHack;
typedef struct xCutscene;
typedef struct xEntMotion;
typedef struct xBound;
typedef struct iFogParams;
typedef struct xGoal;
typedef struct xShadowSimplePoly;
typedef struct RxObjSpace3DVertex;
typedef struct xPlatformAsset;
typedef struct zNPCNewsFish;
typedef struct RwLLLink;
typedef struct NCINBeNosey;
typedef struct xEntMotionAsset;
typedef struct RwTexture;
typedef struct rxHeapFreeBlock;
typedef struct NCINMap;
typedef enum _tagPadState;
typedef struct xScene;
typedef struct xEntBoulder;
typedef struct RxPipelineNodeTopSortData;
typedef struct RwV2d;
typedef struct zFragParticleAsset;
typedef struct xAnimMultiFileEntry;
typedef struct xParCmdTex;
typedef struct xAnimActiveEffect;
typedef struct xGlobals;
typedef struct _tagEmitOffsetPoint;
typedef struct node_data;
typedef struct zEntHangable;
typedef struct xEntAsset;
typedef struct rxHeapSuperBlockDescriptor;
typedef struct NPARMgmt;
typedef struct NPCConfig;
typedef struct bound_data;
typedef enum en_trantype;
typedef struct xMovePointAsset;
typedef struct HAZNotify;
typedef struct zNPCSettings;
typedef struct tri_data_0;
typedef struct _class_1;
typedef struct tri_data_1;
typedef struct xAnimFile;
typedef struct xCutsceneBreak;
typedef struct iEnv;
typedef struct zEntDestructObj;
typedef struct xParEmitter;
typedef struct _tagxPad;
typedef struct xLightKitLight;
typedef struct RxPipelineNodeParam;
typedef struct NCINArc;
typedef struct xAnimSingle;
typedef struct RpVertexNormal;
typedef struct xEntSplineData;
typedef struct HAZCloud;
typedef struct RxHeap;
typedef struct NPCTarget;
typedef struct RwBBox;
typedef struct RwRGBA;
typedef struct NPARXtraData;
typedef struct zNPCBPlankton;
typedef struct xShadowSimpleCache;
typedef struct xCollis;
typedef struct xEnv;
typedef struct XCSNNosey;
typedef struct zFragLightning;
typedef struct xCutsceneMgr;
typedef struct xEntCollis;
typedef struct xQuat;
typedef struct xEntMotionPenData;
typedef struct xPSYNote;
typedef struct RwV3d;
typedef struct UVAModelInfo;
typedef struct zFragBone;
typedef struct zFragLightningAsset;
typedef struct xFactoryInst;
typedef struct rxHeapBlockHeader;
typedef struct _tagPadAnalog;
typedef struct xParEmitterPropsAsset;
typedef struct RpInterpolator;
typedef struct xCutsceneMgrAsset;
typedef struct xAnimMultiFileBase;
typedef struct xJSPHeader;
typedef struct xPEVCyl;
typedef struct zScene;
typedef struct zNPCB_SB2;
typedef struct xUpdateCullMgr;
typedef struct xAnimEffect;
typedef struct _anon0;
typedef enum en_ncinfx;
typedef struct zFragAsset;
typedef struct xEntMPData;
typedef struct xPar;
typedef struct zPlatform;
typedef struct NPCSndQueue;
typedef struct NCINPar;
typedef struct _class_2;
typedef struct xLinkAsset;
typedef struct _class_3;
typedef struct xGroupAsset;
typedef struct xParInterp;
typedef struct zNPCCommon;
typedef struct RpPolygon;
typedef struct _tagiPad;
typedef struct zShrapnelAsset;
typedef struct zMovePoint;
typedef struct xMovePoint;
typedef struct xAnimPlay;
typedef struct NPARData;
typedef struct RpMaterialList;
typedef struct xParCmdAsset;
typedef struct xVec4;
typedef union zFragLocInfo;
typedef struct tag_iFile;
typedef struct xFFX;
typedef struct xModelPool;
typedef struct RwSurfaceProperties;
typedef struct xSpline3;
typedef struct xAnimMultiFile;
typedef struct RpMaterial;
typedef struct xEntMotionERData;
typedef struct xPEEntBone;
typedef struct rxReq;
typedef struct zFragSound;
typedef struct _class_4;
typedef struct _class_5;
typedef struct _class_6;
typedef struct tag_xFile;
typedef struct xPsyche;
typedef struct HAZCatProd;
typedef struct NCINEnts;
typedef struct zFragSoundAsset;
typedef enum en_NPC_SOUND;
typedef enum RxClusterValidityReq;
typedef struct xEntNPCAsset;
typedef enum slug_stage;
typedef struct _class_7;
typedef struct xEntShadow;
typedef struct RpWorldSector;
typedef struct xSurface;
typedef struct zJumpParam;
typedef struct zGlobals;
typedef struct anim_coll_data;
typedef struct zFragLocation;
typedef enum RxNodeDefEditable;
typedef struct xDynAsset;
typedef struct RpMorphTarget;
typedef enum RxClusterValid;
typedef struct xQCData;
typedef struct zGlobalSettings;
typedef struct xPECircle;
typedef struct xRot;
typedef struct zAssetPickupTable;
typedef struct NCINMat;
typedef struct xClumpCollBSPVertInfo;
typedef enum hand_enum;
typedef struct NCINShrap;
typedef struct xClumpCollBSPBranchNode;
typedef struct zFragShockwave;
typedef struct move_config;
typedef struct _tagLightningLine;
typedef struct slug_data;
typedef struct zPlayerLassoInfo;
typedef struct HAZMissile;
typedef enum rxEmbeddedPacketState;
typedef struct xSphere;
typedef struct xClumpCollBSPTriangle;
typedef union NCINData;
typedef struct NPCCone;
typedef struct xEntMotionOrbitData;
typedef struct xGroup;
typedef struct zLedgeGrabParams;
typedef enum RwCameraProjection;
typedef struct RpMeshHeader;
typedef enum RxClusterForcePresent;
typedef enum zFragType;
typedef struct xCylinder;
typedef struct xCutsceneInfo;
typedef struct xMemPool;
typedef union RxColorUnion;
typedef union _class_8;
typedef struct xBox;
typedef enum _zPlayerWallJumpState;
typedef enum en_pendtype;
typedef struct NCINLyt;
typedef struct RxClusterDefinition;
typedef struct NCINCustom;
typedef struct RyzMemData;
typedef struct _tagEmitSphere;
typedef struct platform_data;
typedef struct HAZPatriot;
typedef struct RpTriangle;
typedef struct _tagLightningRot;
typedef struct move_data;
typedef struct hand_data;
typedef enum _tagRumbleType;
typedef struct RwSphere;
typedef struct xCutsceneTime;
typedef enum _zPlayerType;
typedef struct xEntMechData;
typedef union _class_9;
typedef struct xEntMotionSplineData;
typedef struct xListItem_0;
typedef struct HAZTypical;
typedef struct RwTexDictionary;
typedef struct xCutsceneData;
typedef struct RxOutputSpec;
typedef struct analog_data;
typedef struct NCINHaz;
typedef struct zFragGroup;
typedef enum en_hazcol;
typedef struct NCINRast;
typedef struct _tagEmitRect;
typedef struct zLasso;
typedef struct xMat3x3;
typedef struct _class_10;
typedef enum en_nparptyp;
typedef union zFragInfo;
typedef struct RxClusterRef;
typedef struct xListItem_1;
typedef struct xModelTag;
typedef struct RwObject;
typedef struct zCheckPoint;
typedef struct HAZCollide;
typedef struct xEntMotionMPData;
typedef struct xEntERData;
typedef struct zPlayerGlobals;
typedef struct _tagLightningZeus;
typedef struct RxIoSpec;
typedef struct xBaseAsset;
typedef enum RpWorldRenderOrder;
typedef struct _class_11;
typedef struct xEntDrive;
typedef struct xParabola;
typedef struct xShadowPoly;
typedef struct RxNodeMethods;
typedef struct zPlayerCarryInfo;
typedef struct zFragParticle;
typedef struct HAZBall;
typedef struct _class_12;
typedef struct xPEEntBound;
typedef enum _SDRenderState;
typedef enum PSY_BRAIN_STATUS;
typedef struct xVec2;
typedef struct _tagEmitLine;
typedef enum en_npcbtyp;
typedef struct xModelBucket;
typedef struct RwFrustumPlane;
typedef struct xCoef;
typedef enum en_npctgt;
typedef struct RwPlane;
typedef struct xBBox;
typedef struct xEntMotionMechData;
typedef enum en_GOALSTATE;
typedef struct xNPCBasic;
typedef enum move_enum;
typedef struct _class_13;
typedef struct RxCluster;
typedef struct zNPCBoss;
typedef struct xParEmitterCustomSettings;
typedef struct xCutsceneZbuffer;
typedef struct xCamera;
typedef struct _anon1;
typedef struct xEntPenData;
typedef struct HAZRing;

typedef s32(*type_0)(RxPipelineNode*, RxPipelineNodeParam*);
typedef void(*type_1)(xEnt*);
typedef void(*type_2)(s32*, en_trantype*);
typedef void(*type_7)(xEnt*);
typedef s32(*type_9)(RxNodeDefinition*);
typedef void(*type_11)(xMemPool*, void*);
typedef void(*type_14)(zFrag*, f32);
typedef void(*type_22)(RxNodeDefinition*);
typedef void(*type_24)(RwResEntry*);
typedef RwCamera*(*type_29)(RwCamera*);
typedef void(*type_32)(void*);
typedef s32(*type_33)(RxPipelineNode*);
typedef RwCamera*(*type_35)(RwCamera*);
typedef RwObjectHasFrame*(*type_38)(RwObjectHasFrame*);
typedef void(*type_42)(RxPipelineNode*);
typedef RpWorldSector*(*type_52)(RpWorldSector*);
typedef s32(*type_54)(RxPipelineNode*, RxPipeline*);
typedef void(*type_58)(void*, xParGroup*);
typedef void(*type_61)(xEnt*, xScene*, f32);
typedef void(*type_67)(xEnt*, xScene*, f32, xEntCollis*);
typedef u32(*type_68)(u32, xAnimActiveEffect*, xAnimSingle*, void*);
typedef void(*type_69)(xEnt*, xVec3*);
typedef void(*type_73)(xEnt*, xScene*, f32, xEntFrame*);
typedef xBase*(*type_75)(u32);
typedef u32(*type_76)(RxPipelineNode*, u32, u32, void*);
typedef void(*type_80)(xEnt*);
typedef s8*(*type_82)(xBase*);
typedef s8*(*type_89)(u32);
typedef void(*type_91)(zFrag*, zFragAsset*);
typedef u32(*type_100)(xEnt*, xEnt*, xScene*, f32, xCollis*);
typedef u32(*type_104)(xAnimTransition*, xAnimSingle*, void*);
typedef void(*type_107)(xEnt*, xVec3*, xMat4x3*);
typedef void(*type_111)(zShrapnelAsset*, xModelInstance*, xVec3*, type_91);
typedef void(*type_121)(xAnimState*, xAnimSingle*, void*);
typedef void(*type_135)(zCutsceneMgr*, NCINEntry*, s32);
typedef RpClump*(*type_137)(RpClump*, void*);
typedef u32(*type_139)(void*, void*);
typedef void(*type_140)(zCutsceneMgr*, NCINEntry*);
typedef s32(*type_143)(xGoal*, void*, f32, void*);
typedef void(*type_150)(xAnimPlay*, xAnimState*);
typedef void(*type_154)(zCutsceneMgr*, NCINEntry*, RpAtomic*, RwMatrixTag*, u32, u32);
typedef s32(*type_157)(xGoal*, void*, en_trantype*, f32, void*);
typedef void(*type_159)(xAnimPlay*, xQuat*, xVec3*, s32);
typedef s32(*type_164)(xBase*, xBase*, u32, f32*, xBase*);
typedef s32(*type_178)(xGoal*, void*, en_trantype*, f32, void*);
typedef RpAtomic*(*type_183)(RpAtomic*);

typedef u32 type_3[72];
typedef s8 type_4[4];
typedef xMat4x3* type_5[4];
typedef f32 type_6[2];
typedef u8 type_8[2];
typedef u32 type_10[2];
typedef xBase* type_12[72];
typedef RxObjSpace3DVertex type_13[4];
typedef type_13 type_15[2];
typedef u8 type_16[2];
typedef u8 type_17[3];
typedef xVec3 type_18[2];
typedef f32 type_19[6];
typedef NCINEntry type_20[4];
typedef NPCSndQueue type_21[4];
typedef xModelInstance* type_23[2];
typedef f32 type_25[5];
typedef f32 type_26[3];
typedef RxCluster type_27[1];
typedef f32 type_28[3];
typedef xModelTag type_30[2];
typedef xVec4 type_31[12];
typedef NCINEntry type_34[2];
typedef f32 type_36[3];
typedef xEnt* type_37[16];
typedef xShadowPoly type_39[256];
typedef f32 type_40[12];
typedef xVec3 type_41[3];
typedef xCutsceneZbuffer type_43[4];
typedef f32 type_44[12];
typedef s8 type_45[128];
typedef f32 type_46[12];
typedef u32 type_47[2];
typedef xEnt* type_48[111];
typedef f32 type_49[4];
typedef f32 type_50[12];
typedef u32 type_51[2];
typedef s8 type_53[128];
typedef f32 type_55[4];
typedef f32 type_56[12];
typedef NCINEntry type_57[3];
typedef xModelTag type_59[20];
typedef u32 type_60[2];
typedef f32 type_62[12];
typedef NCINEntry type_63[2];
typedef xVec3 type_64[9];
typedef NCINEntry type_65[2];
typedef f32 type_66[16];
typedef s32 type_70[5];
typedef s8 type_71[128];
typedef RwFrustumPlane type_72[6];
typedef type_71 type_74[6];
typedef f32 type_77[2];
typedef NCINEntry type_78[1];
typedef xModelInstance* type_79[4];
typedef u8 type_81[2];
typedef NCINEntry type_83[9];
typedef RpLight* type_84[2];
typedef node_data type_85[9];
typedef u8 type_86[3];
typedef RwV3d type_87[8];
typedef u8 type_88[14];
typedef xModelTag type_90[4];
typedef NCINEntry type_92[2];
typedef hand_data type_93[2];
typedef RwFrame* type_94[2];
typedef NCINEntry type_95[7];
typedef s8 type_96[32];
typedef NCINEntry type_97[27];
typedef NCINEntry type_98[10];
typedef xModelInstance* type_99[14];
typedef platform_data type_101[16];
typedef u8 type_102[4];
typedef s8 type_103[3];
typedef f32 type_105[2];
typedef bound_data type_106[5];
typedef s32 type_108[6];
typedef u8 type_109[3];
typedef u16 type_110[3];
typedef slug_data type_112[3];
typedef u8 type_113[3];
typedef u8 type_114[4];
typedef f32 type_115[2];
typedef u8 type_116[2];
typedef xVec3 type_117[4];
typedef s8 type_118[128];
typedef xAnimMultiFileEntry type_119[1];
typedef f32 type_120[2];
typedef xVec3 type_122[16];
typedef f32 type_123[2];
typedef xLightKitLight type_124[8];
typedef xEnt* type_125[4];
typedef xVec3 type_126[16];
typedef xModelTag type_127[3];
typedef NCINEntry type_128[6];
typedef u8 type_129[22];
typedef NCINEntry type_130[5];
typedef f32 type_131[4];
typedef NCINEntry type_132[10];
typedef s8 type_133[16];
typedef u8 type_134[4];
typedef u8 type_136[22];
typedef f32 type_138[16];
typedef NCINEntry type_141[1];
typedef NCINMap type_142[24];
typedef s8 type_144[16];
typedef s8 type_145[32];
typedef NCINEntry type_146[31];
typedef RwTexCoords* type_147[8];
typedef f32 type_148[3];
typedef f32 type_149[2];
typedef xCollis type_151[18];
typedef u8 type_152[2];
typedef u8 type_153[2];
typedef NCINEntry type_155[2];
typedef u8 type_156[3];
typedef NCINEntry type_158[4];
typedef f32 type_160[2];
typedef xVec3 type_161[60];
typedef NCINEntry type_162[8];
typedef xParInterp type_163[1];
typedef f32 type_165[1];
typedef NCINEntry type_166[3];
typedef f32 type_167[4];
typedef f32 type_168[2];
typedef xVec3 type_169[5];
typedef xVec3 type_170[2];
typedef u16 type_171[3];
typedef f32 type_172[22];
typedef f32 type_173[4];
typedef f32 type_174[15];
typedef f32 type_175[4];
typedef u32 type_176[4];
typedef f32 type_177[2];
typedef f32 type_179[16];
typedef u8 type_180[5];
typedef f32 type_181[4];
typedef f32 type_182[15];
typedef f32 type_184[22];
typedef f32 type_185[4];
typedef NCINEntry type_186[24];
typedef xVec3 type_187[2];
typedef u32 type_188[15];
typedef NCINEntry type_189[4];
typedef xModelTag type_190[4];
typedef xVec3 type_191[2];
typedef u8 type_192[2];
typedef u32 type_193[15];
typedef xModelTag type_194[4];
typedef NCINEntry type_195[28];
typedef u32 type_196[15];
typedef zFrag* type_197[21];
typedef NCINEntry type_198[5];
typedef xVec3 type_199[2];
typedef xParInterp type_200[4];
typedef s8 type_201[16];
typedef NCINEntry type_202[2];
typedef u32 type_203[15];
typedef xParInterp type_204[4];
typedef u32 type_205[2];
typedef s8 type_206[16];
typedef u8 type_207[3];
typedef u32 type_208[4];
typedef f32 type_209[2];
typedef analog_data type_210[2];
typedef f32 type_211[2];
typedef xGoal* type_212[5];
typedef xVec3 type_213[3];
typedef f32 type_214[2];
typedef f32 type_215[1];
typedef type_215 type_216[5];
typedef NCINEntry type_217[1];
typedef f32 type_218[2];
typedef RwTexCoords* type_219[8];
typedef f32 type_220[4];
typedef s8 type_221[32];
typedef f32 type_222[4];
typedef s8 type_223[32];

struct NCINDRay
{
	RwRaster* rast;
};

struct zCutsceneMgr : xCutsceneMgr
{
};

struct _tagLightningAdd
{
	u32 type;
	f32 setup_degrees;
	f32 move_degrees;
	f32 rot_radius;
	xVec3* start;
	xVec3* end;
	s16 total_points;
	s16 end_points;
	f32 time;
	f32 arc_height;
	f32 thickness;
	iColor_tag color;
	f32 rand_radius;
	u32 flags;
	f32 zeus_normal_offset;
	f32 zeus_back_offset;
	f32 zeus_side_offset;
};

struct RpGeometry
{
	RwObject object;
	u32 flags;
	u16 lockedSinceLastInst;
	s16 refCount;
	s32 numTriangles;
	s32 numVertices;
	s32 numMorphTargets;
	s32 numTexCoordSets;
	RpMaterialList matList;
	RpTriangle* triangles;
	RwRGBA* preLitLum;
	type_219 texCoords;
	RpMeshHeader* mesh;
	RwResEntry* repEntry;
	RpMorphTarget* morphTarget;
};

struct xBase
{
	u32 id;
	u8 baseType;
	u8 linkCount;
	u16 baseFlags;
	xLinkAsset* link;
	type_164 eventFunc;
};

struct xUpdateCullEnt
{
	u16 index;
	s16 groupIndex;
	type_139 cb;
	void* cbdata;
	xUpdateCullEnt* nextInGroup;
};

struct xShadowCache
{
	xVec3 pos;
	f32 radius;
	u32 entCount;
	u32 polyCount;
	type_25 polyRayDepth;
	u16 castOnEnt;
	u16 castOnPoly;
	type_37 ent;
	type_39 poly;
};

struct xLightKit
{
	u32 tagID;
	u32 groupID;
	u32 lightCount;
	xLightKitLight* lightList;
};

struct xVec3
{
	f32 x;
	f32 y;
	f32 z;
};

struct NCINEntry
{
	en_ncinfx typ_ncinfx;
	type_135 cb_fxupd;
	type_154 cb_fxanim;
	type_140 cb_fxrend;
	f32 tym_beg;
	f32 tym_end;
	type_187 pos_A;
	type_191 pos_B;
	s8* twk_name;
	s32 idx_anim;
	s32 idx_bone;
	s32 flg_stat;
	NCINData fxdata;
};

struct xModelInstance
{
	xModelInstance* Next;
	xModelInstance* Parent;
	xModelPool* Pool;
	xAnimPlay* Anim;
	RpAtomic* Data;
	u32 PipeFlags;
	f32 RedMultiplier;
	f32 GreenMultiplier;
	f32 BlueMultiplier;
	f32 Alpha;
	f32 FadeStart;
	f32 FadeEnd;
	xSurface* Surf;
	xModelBucket** Bucket;
	xModelInstance* BucketNext;
	xLightKit* LightKit;
	void* Object;
	u16 Flags;
	u8 BoneCount;
	u8 BoneIndex;
	u8* BoneRemap;
	RwMatrixTag* Mat;
	xVec3 Scale;
	u32 modelID;
	u32 shadowID;
	RpAtomic* shadowmapAtomic;
	_class_1 anim_coll;
};

struct RwFrame
{
	RwObject object;
	RwLLLink inDirtyListLink;
	RwMatrixTag modelling;
	RwMatrixTag ltm;
	RwLinkList objectList;
	RwFrame* child;
	RwFrame* next;
	RwFrame* root;
};

struct _class_0
{
	xVec3 body;
	xVec3 mouth;
	type_18 hand;
};

enum en_LASSO_STATUS
{
	LASS_STAT_DONE,
	LASS_STAT_PENDING,
	LASS_STAT_GRABBING,
	LASS_STAT_TOSSING,
	LASS_STAT_NOMORE,
	LASS_STAT_FORCEINT = 0x7fffffff
};

struct RxPipeline
{
	s32 locked;
	u32 numNodes;
	RxPipelineNode* nodes;
	u32 packetNumClusterSlots;
	rxEmbeddedPacketState embeddedPacketState;
	RxPacket* embeddedPacket;
	u32 numInputRequirements;
	RxPipelineRequiresCluster* inputRequirements;
	void* superBlock;
	u32 superBlockSize;
	u32 entryPoint;
	u32 pluginId;
	u32 pluginData;
};

struct RwMatrixTag
{
	RwV3d right;
	u32 flags;
	RwV3d up;
	u32 pad1;
	RwV3d at;
	u32 pad2;
	RwV3d pos;
	u32 pad3;
};

struct RpSkin
{
};

struct xModelAssetParam
{
};

struct zFragProjectile
{
	zFragProjectileAsset* fasset;
	xModelInstance* model;
	xParabola path;
	f32 angVel;
	f32 t;
	f32 tColl;
	s32 numBounces;
	f32 scale;
	f32 parentScale;
	f32 alpha;
	xVec3 N;
	xVec3 axis;
};

struct RxPacket
{
	u16 flags;
	u16 numClusters;
	RxPipeline* pipeline;
	u32* inputToClusterSlot;
	u32* slotsContinue;
	RxPipelineCluster** slotClusterRefs;
	type_27 clusters;
};

struct RpLight
{
	RwObjectHasFrame object;
	f32 radius;
	RwRGBAReal color;
	f32 minusCosAngle;
	RwLinkList WorldSectorsInLight;
	RwLLLink inWorld;
	u16 lightFrame;
	u16 pad;
};

struct RwCamera
{
	RwObjectHasFrame object;
	RwCameraProjection projectionType;
	type_29 beginUpdate;
	type_35 endUpdate;
	RwMatrixTag viewMatrix;
	RwRaster* frameBuffer;
	RwRaster* zBuffer;
	RwV2d viewWindow;
	RwV2d recipViewWindow;
	RwV2d viewOffset;
	f32 nearPlane;
	f32 farPlane;
	f32 fogPlane;
	f32 zScale;
	f32 zShift;
	type_72 frustumPlanes;
	RwBBox frustumBoundBox;
	type_87 frustumCorners;
};

struct RwRaster
{
	RwRaster* parent;
	u8* cpPixels;
	u8* palette;
	s32 width;
	s32 height;
	s32 depth;
	s32 stride;
	s16 nOffsetX;
	s16 nOffsetY;
	u8 cType;
	u8 cFlags;
	u8 privateFlags;
	u8 cFormat;
	u8* originalPixels;
	s32 originalWidth;
	s32 originalHeight;
	s32 originalStride;
};

struct zPlayerSettings
{
	_zPlayerType pcType;
	type_19 MoveSpeed;
	type_26 AnimSneak;
	type_28 AnimWalk;
	type_36 AnimRun;
	f32 JumpGravity;
	f32 GravSmooth;
	f32 FloatSpeed;
	f32 ButtsmashSpeed;
	zJumpParam Jump;
	zJumpParam Bounce;
	zJumpParam Spring;
	zJumpParam Wall;
	zJumpParam Double;
	zJumpParam SlideDouble;
	zJumpParam SlideJump;
	f32 WallJumpVelocity;
	zLedgeGrabParams ledge;
	f32 spin_damp_xz;
	f32 spin_damp_y;
	u8 talk_anims;
	u8 talk_filter_size;
	type_114 talk_filter;
};

struct RxPipelineCluster
{
	RxClusterDefinition* clusterRef;
	u32 creationAttributes;
};

struct xMat4x3 : xMat3x3
{
	xVec3 pos;
	u32 pad3;
};

struct RpWorld
{
	RwObject object;
	u32 flags;
	RpWorldRenderOrder renderOrder;
	RpMaterialList matList;
	RpSector* rootSector;
	s32 numTexCoordSets;
	s32 numClumpsInWorld;
	RwLLLink* currentClumpLink;
	RwLinkList clumpList;
	RwLinkList lightList;
	RwLinkList directionalLightList;
	RwV3d worldOrigin;
	RwBBox boundingBox;
	type_52 renderCallBack;
	RxPipeline* pipeline;
};

struct RpAtomic
{
	RwObjectHasFrame object;
	RwResEntry* repEntry;
	RpGeometry* geometry;
	RwSphere boundingSphere;
	RwSphere worldBoundingSphere;
	RpClump* clump;
	RwLLLink inClumpLink;
	type_183 renderCallBack;
	RpInterpolator interpolator;
	u16 renderFrame;
	u16 pad;
	RwLinkList llWorldSectorsInAtomic;
	RxPipeline* pipeline;
};

struct RxNodeDefinition
{
	s8* name;
	RxNodeMethods nodeMethods;
	RxIoSpec io;
	u32 pipelineNodePrivateDataSize;
	RxNodeDefEditable editable;
	s32 InputPipesCnt;
};

struct zFragProjectileAsset : zFragAsset
{
	u32 modelInfoID;
	RpAtomic* modelFile;
	zFragLocation launch;
	zFragLocation vel;
	f32 bounce;
	s32 maxBounces;
	u32 flags;
	u32 childID;
	zShrapnelAsset* child;
	f32 minScale;
	f32 maxScale;
	u32 scaleCurveID;
	xCurveAsset* scaleCurve;
	f32 gravity;
};

struct xParEmitterAsset : xBaseAsset
{
	u8 emit_flags;
	u8 emit_type;
	u16 pad;
	u32 propID;
	union
	{
		xPECircle e_circle;
		_tagEmitSphere e_sphere;
		_tagEmitRect e_rect;
		_tagEmitLine e_line;
		_tagEmitVolume e_volume;
		_tagEmitOffsetPoint e_offsetp;
		xPEVCyl e_vcyl;
		xPEEntBone e_entbone;
		xPEEntBound e_entbound;
	};
	u32 attachToID;
	xVec3 pos;
	xVec3 vel;
	f32 vel_angle_variation;
	u32 cull_mode;
	f32 cull_dist_sqr;
};

struct NPCHazard
{
	en_npchaz typ_hazard;
	s32 flg_hazard;
	xVec3 pos_hazard;
	xModelInstance* mdl_hazard;
	UVAModelInfo* uva_uvanim;
	f32 tym_lifespan;
	f32 tmr_remain;
	f32 pam_interp;
	union
	{
		f32 tmr_generic;
		f32 tmr_nextglob;
	};
	union
	{
		s32 cnt_generic;
		s32 cnt_nextemit;
		s32 flg_casthurt;
	};
	_class_8 custdata;
	HAZNotify* cb_notify;
	zNPCCommon* npc_owner;
	NPCHazard* haz_parent;
	xShadowCache* shadowCache;
};

struct _tagxRumble
{
	_tagRumbleType type;
	f32 seconds;
	_tagxRumble* next;
	s16 active;
	u16 fxflags;
};

struct xModelTagWithNormal : xModelTag
{
	xVec3 normal;
};

struct xAnimTransitionList
{
	xAnimTransitionList* Next;
	xAnimTransition* T;
};

struct zParEmitter : xParEmitter
{
};

struct zLightning
{
	u32 type;
	u32 flags;
	union
	{
		_class_5 legacy;
		_class_10 func;
	};
	iColor_tag color;
	f32 time_left;
	f32 time_total;
};

struct RxPipelineRequiresCluster
{
	RxClusterDefinition* clusterDef;
	RxClusterValidityReq rqdOrOpt;
	u32 slotIndex;
};

struct zEnt : xEnt
{
	xAnimTable* atbl;
};

struct zNPCLassoInfo
{
	en_LASSO_STATUS stage;
	xEnt* lassoee;
	xAnimState* holdGuideAnim;
	xModelInstance* holdGuideModel;
	xAnimState* grabGuideAnim;
	xModelInstance* grabGuideModel;
};

enum RwFogType
{
	rwFOGTYPENAFOGTYPE,
	rwFOGTYPELINEAR,
	rwFOGTYPEEXPONENTIAL,
	rwFOGTYPEEXPONENTIAL2,
	rwFOGTYPEFORCEENUMSIZEINT = 0x7fffffff
};

struct RwTexCoords
{
	f32 u;
	f32 v;
};

struct RwResEntry
{
	RwLLLink link;
	s32 size;
	void* owner;
	RwResEntry** ownerRef;
	type_24 destroyNotify;
};

struct xEntFrame
{
	xMat4x3 mat;
	xMat4x3 oldmat;
	xVec3 oldvel;
	xRot oldrot;
	xRot drot;
	xRot rot;
	xVec3 dpos;
	xVec3 dvel;
	xVec3 vel;
	u32 mode;
};

struct xCoef3
{
	xCoef x;
	xCoef y;
	xCoef z;
};

struct NPCSndTrax
{
	en_NPC_SOUND typ_sound;
	s8* nam_sound;
	u32 aid_sound;
};

struct xAnimTable
{
	xAnimTable* Next;
	s8* Name;
	xAnimTransition* TransitionList;
	xAnimState* StateList;
	u32 AnimIndex;
	u32 MorphIndex;
	u32 UserFlags;
};

struct _zPortal : xBase
{
	xPortalAsset* passet;
};

struct zFragShockwaveAsset : zFragAsset
{
	u32 modelInfoID;
	f32 birthRadius;
	f32 deathRadius;
	f32 birthVelocity;
	f32 deathVelocity;
	f32 birthSpin;
	f32 deathSpin;
	type_49 birthColor;
	type_55 deathColor;
};

struct iColor_tag
{
	u8 r;
	u8 g;
	u8 b;
	u8 a;
};

struct _tagEmitVolume
{
	u32 emit_volumeID;
};

struct xParSysAsset : xBaseAsset
{
	u32 type;
	u32 parentParSysID;
	u32 textureID;
	u8 parFlags;
	u8 priority;
	u16 maxPar;
	u8 renderFunc;
	u8 renderSrcBlendMode;
	u8 renderDstBlendMode;
	u8 cmdCount;
	u32 cmdSize;
};

struct RpClump
{
	RwObject object;
	RwLinkList atomicList;
	RwLinkList lightList;
	RwLinkList cameraList;
	RwLLLink inWorldLink;
	type_137 callback;
};

struct NCINLamp
{
	xVec3 pos_robo;
	RwRaster* rast;
};

struct xCurveAsset
{
};

struct _zEnv : xBase
{
	xEnvAsset* easset;
};

struct xEntOrbitData
{
	xVec3 orig;
	xVec3 c;
	f32 a;
	f32 b;
	f32 p;
	f32 w;
};

struct RwRGBAReal
{
	f32 red;
	f32 green;
	f32 blue;
	f32 alpha;
};

struct HAZTarTar : HAZCollide
{
	xVec3 vel;
	f32 spd_lob;
	xVec3 pos_tgt;
	u32 streakID;
};

struct RwObjectHasFrame
{
	RwObject object;
	RwLLLink lFrame;
	type_38 sync;
};

enum en_dupowavmod
{
	NPCP_DUPOWAVE_CONTINUOUS,
	NPCP_DUPOWAVE_DISCREET,
	NPCP_DUPOWAVE_NOMORE,
	NPCP_DUPOWAVE_FORCE = 0x7fffffff
};

struct xAnimTransition
{
	xAnimTransition* Next;
	xAnimState* Dest;
	type_104 Conditional;
	type_104 Callback;
	u32 Flags;
	u32 UserFlags;
	f32 SrcTime;
	f32 DestTime;
	u16 Priority;
	u16 QueuePriority;
	f32 BlendRecip;
	u16* BlendOffset;
};

struct xClumpCollBSPTree
{
	u32 numBranchNodes;
	xClumpCollBSPBranchNode* branchNodes;
	u32 numTriangles;
	xClumpCollBSPTriangle* triangles;
};

struct xGridBound
{
	void* data;
	u16 gx;
	u16 gz;
	u8 ingrid;
	u8 oversize;
	u8 deleted;
	u8 gpad;
	xGridBound** head;
	xGridBound* next;
};

struct xParCmd
{
	u32 flag;
	xParCmdAsset* tasset;
};

struct xPortalAsset : xBaseAsset
{
	u32 assetCameraID;
	u32 assetMarkerID;
	f32 ang;
	u32 sceneID;
};

struct zFrag
{
	zFragType type;
	zFragInfo info;
	f32 delay;
	f32 alivetime;
	f32 lifetime;
	type_14 update;
	type_23 parent;
	zFrag* prev;
	zFrag* next;
};

enum zFragLocType
{
	eFragLocBone,
	eFragLocBoneUpdated,
	eFragLocBoneLocal,
	eFragLocBoneLocalUpdated,
	eFragLocTag,
	eFragLocTagUpdated,
	eFragLocCount,
	eFragLocForceSize = 0x7fffffff
};

struct xEnt : xBase
{
	xEntAsset* asset;
	u16 idx;
	u16 num_updates;
	u8 flags;
	u8 miscflags;
	u8 subType;
	u8 pflags;
	u8 moreFlags;
	u8 isCulled;
	u8 driving_count;
	u8 num_ffx;
	u8 collType;
	u8 collLev;
	u8 chkby;
	u8 penby;
	xModelInstance* model;
	xModelInstance* collModel;
	xModelInstance* camcollModel;
	xLightKit* lightKit;
	type_61 update;
	type_61 endUpdate;
	type_69 bupdate;
	type_73 move;
	type_80 render;
	xEntFrame* frame;
	xEntCollis* collis;
	xGridBound gridb;
	xBound bound;
	type_107 transl;
	xFFX* ffx;
	xEnt* driver;
	s32 driveMode;
	xShadowSimpleCache* simpShadow;
	xEntShadow* entShadow;
	anim_coll_data* anim_coll;
	void* user_data;
};

struct xUpdateCullGroup
{
	u32 active;
	u16 startIndex;
	u16 endIndex;
	xGroup* groupObject;
};

struct RpSector
{
	s32 type;
};

struct RwLinkList
{
	RwLLLink link;
};

struct NCINStrk
{
	u32 sid_vert;
	u32 sid_horz;
};

struct xParSys : xBase
{
	xParSysAsset* tasset;
	u32 cmdCount;
	xParCmd* cmd;
	xParSys* parent;
	xParGroup* group;
	u8 visible;
	RwTexture* txtr_particle;
};

struct xJSPNodeInfo
{
	s32 originalMatIndex;
	s32 nodeFlags;
};

struct HAZShroom : HAZTypical
{
	xVec3 vel_rise;
	xVec3 acc_rise;
};

enum en_npchaz
{
	NPC_HAZ_UNKNOWN,
	NPC_HAZ_EXPLODE,
	NPC_HAZ_EXPLODE_INNER,
	NPC_HAZ_FODBOMB,
	NPC_HAZ_CATTLEPROD,
	NPC_HAZ_TUBELETBLAST,
	NPC_HAZ_PUPPYNUKE,
	NPC_HAZ_DUPLOBOOM,
	NPC_HAZ_DUPLO_SHROOM,
	NPC_HAZ_PATRIOT,
	NPC_HAZ_TARTARPROJ,
	NPC_HAZ_TARTARSPILL,
	NPC_HAZ_TARTARSTINK,
	NPC_HAZ_CHUCKBOMB,
	NPC_HAZ_CHUCKBLAST,
	NPC_HAZ_CHUCKBLOOSH,
	NPC_HAZ_ARFBONE,
	NPC_HAZ_ARFBONEBLAST,
	NPC_HAZ_OILBUBBLE,
	NPC_HAZ_OILSLICK,
	NPC_HAZ_OILBURST,
	NPC_HAZ_OILGLOB,
	NPC_HAZ_MONCLOUD,
	NPC_HAZ_FUNFRAG,
	NPC_HAZ_THUNDER,
	NPC_HAZ_ROBOBITS,
	NPC_HAZ_VISSPLASH,
	NPC_HAZ_NOMORE,
	NPC_HAZ_FORCE = 0x7fffffff
};

struct RxPipelineNode
{
	RxNodeDefinition* nodeDef;
	u32 numOutputs;
	u32* outputs;
	RxPipelineCluster** slotClusterRefs;
	u32* slotsContinue;
	void* privateData;
	u32* inputToClusterSlot;
	RxPipelineNodeTopSortData* topSortData;
	void* initializationData;
	u32 initializationDataSize;
};

struct xAnimState
{
	xAnimState* Next;
	s8* Name;
	u32 ID;
	u32 Flags;
	u32 UserFlags;
	f32 Speed;
	xAnimFile* Data;
	xAnimEffect* Effects;
	xAnimTransitionList* Default;
	xAnimTransitionList* List;
	f32* BoneBlend;
	f32* TimeSnap;
	f32 FadeRecip;
	u16* FadeOffset;
	void* CallbackData;
	xAnimMultiFile* MultiFile;
	type_150 BeforeEnter;
	type_121 StateCallback;
	type_159 BeforeAnimMatrices;
};

struct xEnvAsset : xBaseAsset
{
	u32 bspAssetID;
	u32 startCameraAssetID;
	u32 climateFlags;
	f32 climateStrengthMin;
	f32 climateStrengthMax;
	u32 bspLightKit;
	u32 objectLightKit;
	f32 padF1;
	u32 bspCollisionAssetID;
	u32 bspFXAssetID;
	u32 bspCameraAssetID;
	u32 bspMapperID;
	u32 bspMapperCollisionID;
	u32 bspMapperFXID;
	f32 loldHeight;
};

struct zPlatFMRunTime
{
	u32 flags;
	type_40 tmrs;
	type_44 ttms;
	type_46 atms;
	type_50 dtms;
	type_56 vms;
	type_62 dss;
};

struct xParGroup
{
	xPar* m_root;
	xPar* m_dead;
	s32 m_num_of_particles;
	u8 m_alive;
	u8 m_killWhenDead;
	u8 m_active;
	u8 m_visible;
	u8 m_culled;
	u8 m_priority;
	u8 m_flags;
	u8 m_regidx;
	xParGroup* m_next;
	xParGroup* m_prev;
	type_58 draw;
	xParCmdTex* m_cmdTex;
};

struct xCutsceneZbufferHack
{
	s8* name;
	type_43 times;
};

struct xCutscene
{
	xCutsceneInfo* Info;
	xCutsceneData* Data;
	u32* TimeChunkOffs;
	u32* Visibility;
	xCutsceneBreak* BreakList;
	xCutsceneTime* Play;
	xCutsceneTime* Stream;
	u32 Waiting;
	u32 BadReadPause;
	f32 BadReadSpeed;
	void* RawBuf;
	void* AlignBuf;
	f32 Time;
	f32 CamTime;
	u32 PlayIndex;
	u32 Ready;
	s32 DataLoading;
	u32 GotData;
	u32 ShutDownWait;
	f32 PlaybackSpeed;
	u32 Opened;
	tag_xFile File;
	s32 AsyncID;
	void* MemBuf;
	void* MemCurr;
	u32 SndStarted;
	u32 SndNumChannel;
	type_47 SndChannelReq;
	type_51 SndAssetID;
	type_60 SndHandle;
	XCSNNosey* cb_nosey;
};

struct xEntMotion
{
	xEntMotionAsset* asset;
	u8 type;
	u8 pad;
	u16 flags;
	f32 t;
	f32 tmr;
	f32 d;
	union
	{
		xEntERData er;
		xEntOrbitData orb;
		xEntSplineData spl;
		xEntMPData mp;
		xEntMechData mech;
		xEntPenData pen;
	};
	xEnt* owner;
	xEnt* target;
};

struct xBound
{
	xQCData qcd;
	u8 type;
	type_86 pad;
	union
	{
		xSphere sph;
		xBBox box;
		xCylinder cyl;
	};
	xMat4x3* mat;
};

struct iFogParams
{
	RwFogType type;
	f32 start;
	f32 stop;
	f32 density;
	RwRGBA fogcolor;
	RwRGBA bgcolor;
	u8* table;
};

struct xGoal : xListItem_0, xFactoryInst
{
	xPsyche* psyche;
	s32 goalID;
	en_GOALSTATE stat;
	s32 flg_able;
	type_178 fun_process;
	type_143 fun_precalc;
	type_157 fun_chkRule;
	void* cbdata;
};

struct xShadowSimplePoly
{
	type_41 vert;
	xVec3 norm;
};

struct RxObjSpace3DVertex
{
	RwV3d objVertex;
	RxColorUnion c;
	RwV3d objNormal;
	f32 u;
	f32 v;
};

struct xPlatformAsset
{
};

struct zNPCNewsFish
{
};

struct RwLLLink
{
	RwLLLink* next;
	RwLLLink* prev;
};

struct NCINBeNosey : XCSNNosey
{
	zCutsceneMgr* use_csnmgr;
	NCINEntry* use_fxtab;
};

struct xEntMotionAsset
{
	u8 type;
	u8 use_banking;
	u16 flags;
	union
	{
		xEntMotionERData er;
		xEntMotionOrbitData orb;
		xEntMotionSplineData spl;
		xEntMotionMPData mp;
		xEntMotionMechData mech;
		xEntMotionPenData pen;
	};
};

struct RwTexture
{
	RwRaster* raster;
	RwTexDictionary* dict;
	RwLLLink lInDictionary;
	type_221 name;
	type_223 mask;
	u32 filterAddressing;
	s32 refCount;
};

struct rxHeapFreeBlock
{
	u32 size;
	rxHeapBlockHeader* ptr;
};

struct NCINMap
{
	s8* cinname;
	NCINEntry* cinfxtab;
	u32 cinhash;
};

enum _tagPadState
{
	ePad_Disabled,
	ePad_DisabledError,
	ePad_Enabled,
	ePad_Missing,
	ePad_Total
};

struct xScene
{
	u32 sceneID;
	u16 flags;
	u16 num_ents;
	u16 num_trigs;
	u16 num_stats;
	u16 num_dyns;
	u16 num_npcs;
	u16 num_act_ents;
	u16 num_nact_ents;
	f32 gravity;
	f32 drag;
	f32 friction;
	u16 num_ents_allocd;
	u16 num_trigs_allocd;
	u16 num_stats_allocd;
	u16 num_dyns_allocd;
	u16 num_npcs_allocd;
	xEnt** trigs;
	xEnt** stats;
	xEnt** dyns;
	xEnt** npcs;
	xEnt** act_ents;
	xEnt** nact_ents;
	xEnv* env;
	xMemPool mempool;
	type_75 resolvID;
	type_82 base2Name;
	type_89 id2Name;
};

struct xEntBoulder
{
};

struct RxPipelineNodeTopSortData
{
	u32 numIns;
	u32 numInsVisited;
	rxReq* req;
};

struct RwV2d
{
	f32 x;
	f32 y;
};

struct zFragParticleAsset : zFragAsset
{
	zFragLocation source;
	zFragLocation vel;
	xParEmitterCustomSettings emit;
	u32 parEmitterID;
	zParEmitter* parEmitter;
};

struct xAnimMultiFileEntry
{
	u32 ID;
	xAnimFile* File;
};

struct xParCmdTex : xParCmdAsset
{
	f32 x1;
	f32 y1;
	f32 x2;
	f32 y2;
	u8 birthMode;
	u8 rows;
	u8 cols;
	u8 unit_count;
	f32 unit_width;
	f32 unit_height;
};

struct xAnimActiveEffect
{
	xAnimEffect* Effect;
	u32 Handle;
};

struct xGlobals
{
	xCamera camera;
	_tagxPad* pad0;
	_tagxPad* pad1;
	_tagxPad* pad2;
	_tagxPad* pad3;
	s32 profile;
	type_74 profFunc;
	xUpdateCullMgr* updateMgr;
	s32 sceneFirst;
	type_96 sceneStart;
	RpWorld* currWorld;
	iFogParams fog;
	iFogParams fogA;
	iFogParams fogB;
	s32 fog_t0;
	s32 fog_t1;
	s32 option_vibration;
	u32 QuarterSpeed;
	f32 update_dt;
	s32 useHIPHOP;
	u8 NoMusic;
	s8 currentActivePad;
	u8 firstStartPressed;
	u32 minVSyncCnt;
	u8 dontShowPadMessageDuringLoadingOrCutScene;
	u8 autoSaveFeature;
};

struct _tagEmitOffsetPoint
{
	xVec3 offset;
};

struct node_data
{
	zEntDestructObj* ent;
	f32 pulse;
	xLightKit* old_light_kit;
	RpAtomic* skin_model;
	RwMatrixTag* skin_mat;
	union
	{
		_class_2 v2n1;
		_class_7 v3;
	};
};

struct zEntHangable
{
};

struct xEntAsset : xBaseAsset
{
	u8 flags;
	u8 subtype;
	u8 pflags;
	u8 moreFlags;
	u8 pad;
	u32 surfaceID;
	xVec3 ang;
	xVec3 pos;
	xVec3 scale;
	f32 redMult;
	f32 greenMult;
	f32 blueMult;
	f32 seeThru;
	f32 seeThruSpeed;
	u32 modelInfoID;
	u32 animListID;
};

struct rxHeapSuperBlockDescriptor
{
	void* start;
	u32 size;
	rxHeapSuperBlockDescriptor* next;
};

struct NPARMgmt
{
	en_nparptyp typ_npar;
	s32 flg_npar;
	NPARData* par_buf;
	s32 cnt_active;
	s32 num_max;
	RwTexture* txtr;
	NPARXtraData* xtra_data;
	void** user_data;
};

struct NPCConfig : xListItem_1
{
	u32 modelID;
	s32 flg_config;
	f32 spd_turnMax;
	f32 spd_moveMax;
	f32 fac_accelMax;
	f32 fac_driftMax;
	f32 fac_gravKnock;
	f32 fac_elastic;
	s32 pts_damage;
	s32 useBoxBound;
	xVec3 off_bound;
	xVec3 dim_bound;
	f32 npcMass;
	f32 npcMassInv;
	f32 rad_detect;
	f32 hyt_detect;
	f32 off_detect;
	f32 rad_attack;
	f32 fov_attack;
	xVec3 scl_model;
	f32 tym_attack;
	f32 tym_fidget;
	f32 tym_stun;
	f32 tym_alert;
	f32 dst_castShadow;
	f32 rad_shadowCache;
	f32 rad_shadowRaster;
	f32 rad_dmgSize;
	s32 flg_vert;
	type_59 tag_vert;
	type_64 animFrameRange;
	type_70 cnt_esteem;
	f32 rad_sound;
	NPCSndTrax* snd_trax;
	NPCSndTrax* snd_traxShare;
	s32 test_count;
	type_102 talk_filter;
	u8 talk_filter_size;
};

struct bound_data
{
	xEnt ent;
	xMat4x3 mat;
	xMat3x3 rot_mat;
};

enum en_trantype
{
	GOAL_TRAN_NONE,
	GOAL_TRAN_SET,
	GOAL_TRAN_PUSH,
	GOAL_TRAN_POP,
	GOAL_TRAN_POPTO,
	GOAL_TRAN_POPALL,
	GOAL_TRAN_POPBASE,
	GOAL_TRAN_POPSAFE,
	GOAL_TRAN_SWAP,
	GOAL_TRAN_NOMORE,
	GOAL_TRAN_FORCE = 0x7fffffff
};

struct xMovePointAsset : xBaseAsset
{
	xVec3 pos;
	u16 wt;
	u8 on;
	u8 bezIndex;
	u8 flg_props;
	u8 pad;
	u16 numPoints;
	f32 delay;
	f32 zoneRadius;
	f32 arenaRadius;
};

struct HAZNotify
{
};

struct zNPCSettings : xDynAsset
{
	en_npcbtyp basisType;
	s8 allowDetect;
	s8 allowPatrol;
	s8 allowWander;
	s8 reduceCollide;
	s8 useNavSplines;
	type_103 pad;
	s8 allowChase;
	s8 allowAttack;
	s8 assumeLOS;
	s8 assumeFOV;
	en_dupowavmod duploWaveMode;
	f32 duploSpawnDelay;
	s32 duploSpawnLifeMax;
};

struct tri_data_0 : tri_data_1
{
	xVec3 loc;
	f32 yaw;
	xCollis* coll;
};

struct _class_1
{
	xVec3* verts;
};

struct tri_data_1
{
	u32 index;
	f32 r;
	f32 d;
};

struct xAnimFile
{
	xAnimFile* Next;
	s8* Name;
	u32 ID;
	u32 FileFlags;
	f32 Duration;
	f32 TimeOffset;
	u16 BoneCount;
	type_116 NumAnims;
	void** RawData;
};

struct xCutsceneBreak
{
	f32 Time;
	s32 Index;
};

struct iEnv
{
	RpWorld* world;
	RpWorld* collision;
	RpWorld* fx;
	RpWorld* camera;
	xJSPHeader* jsp;
	type_84 light;
	type_94 light_frame;
	s32 memlvl;
};

struct zEntDestructObj
{
};

struct xParEmitter : xBase
{
	xParEmitterAsset* tasset;
	xParGroup* group;
	xParEmitterPropsAsset* prop;
	u8 rate_mode;
	f32 rate;
	f32 rate_time;
	f32 rate_fraction;
	f32 rate_fraction_cull;
	u8 emit_flags;
	type_109 emit_pad;
	type_113 rot;
	xModelTag tag;
	f32 oocull_distance_sqr;
	f32 distance_to_cull_sqr;
	void* attachTo;
	xParSys* parSys;
	void* emit_volume;
	xVec3 last_attach_loc;
};

struct _tagxPad
{
	type_129 value;
	type_136 last_value;
	u32 on;
	u32 pressed;
	u32 released;
	_tagPadAnalog analog1;
	_tagPadAnalog analog2;
	_tagPadState state;
	u32 flags;
	_tagxRumble rumble_head;
	s16 port;
	s16 slot;
	_tagiPad context;
	f32 al2d_timer;
	f32 ar2d_timer;
	f32 d_timer;
	type_172 up_tmr;
	type_184 down_tmr;
	type_210 analog;
};

struct xLightKitLight
{
	u32 type;
	RwRGBAReal color;
	type_66 matrix;
	f32 radius;
	f32 angle;
	RpLight* platLight;
};

struct RxPipelineNodeParam
{
	void* dataParam;
	RxHeap* heap;
};

struct NCINArc
{
	zLightning* lightning;
	xVec3 endPos;
};

struct xAnimSingle
{
	u32 SingleFlags;
	xAnimState* State;
	f32 Time;
	f32 CurrentSpeed;
	type_77 BilinearLerp;
	xAnimEffect* Effect;
	u32 ActiveCount;
	f32 LastTime;
	xAnimActiveEffect* ActiveList;
	xAnimPlay* Play;
	xAnimTransition* Sync;
	xAnimTransition* Tran;
	xAnimSingle* Blend;
	f32 BlendFactor;
	u32 pad;
};

struct RpVertexNormal
{
	s8 x;
	s8 y;
	s8 z;
	u8 pad;
};

struct xEntSplineData
{
	s32 unknown;
};

struct HAZCloud : HAZTypical
{
	f32 spd_cloud;
	xVec3 pos_home;
	f32 rad_maxRange;
	f32 tmr_dozap;
	zLightning* zap_lytnin;
	zLightning* zap_warnin;
	xVec3 pos_warnin;
};

struct RxHeap
{
	u32 superBlockSize;
	rxHeapSuperBlockDescriptor* head;
	rxHeapBlockHeader* headBlock;
	rxHeapFreeBlock* freeBlocks;
	u32 entriesAlloced;
	u32 entriesUsed;
	s32 dirty;
};

struct NPCTarget
{
	en_npctgt typ_target;
	union
	{
		xEnt* ent_target;
		xBase* bas_target;
		xVec3 pos_target;
		zMovePoint* nav_target;
	};
	zNPCCommon* npc_owner;
};

struct RwBBox
{
	RwV3d sup;
	RwV3d inf;
};

struct RwRGBA
{
	u8 red;
	u8 green;
	u8 blue;
	u8 alpha;
};

struct NPARXtraData
{
};

struct zNPCBPlankton
{
};

struct xShadowSimpleCache
{
	u16 flags;
	u8 alpha;
	u8 pad;
	u32 collPriority;
	xVec3 pos;
	xVec3 at;
	xEnt* castOnEnt;
	xShadowSimplePoly poly;
	f32 envHeight;
	f32 shadowHeight;
	u32 raster;
	f32 dydx;
	f32 dydz;
	type_117 corner;
};

struct xCollis
{
	u32 flags;
	u32 oid;
	void* optr;
	xModelInstance* mptr;
	f32 dist;
	xVec3 norm;
	xVec3 tohit;
	xVec3 depen;
	xVec3 hdng;
	union
	{
		_class_3 tuv;
		tri_data_1 tri;
	};
};

struct xEnv
{
	iEnv* geom;
	iEnv ienv;
	xLightKit* lightKit;
};

struct XCSNNosey
{
	void* userdata;
	s32 flg_nosey;
};

struct zFragLightning
{
	zFragLightningAsset* fasset;
	xModelInstance* startParent;
	xModelInstance* endParent;
	zLightning* lightning;
};

struct xCutsceneMgr : xBase
{
	xCutsceneMgrAsset* tasset;
	xCutscene* csn;
	u32 stop;
	xCutsceneZbufferHack* zhack;
	f32 oldfov;
};

struct xEntCollis
{
	u8 chk;
	u8 pen;
	u8 env_sidx;
	u8 env_eidx;
	u8 npc_sidx;
	u8 npc_eidx;
	u8 dyn_sidx;
	u8 dyn_eidx;
	u8 stat_sidx;
	u8 stat_eidx;
	u8 idx;
	type_151 colls;
	type_67 post;
	type_100 depenq;
};

struct xQuat
{
	xVec3 v;
	f32 s;
};

struct xEntMotionPenData
{
	u8 flags;
	u8 plane;
	type_81 pad;
	f32 len;
	f32 range;
	f32 period;
	f32 phase;
};

struct xPSYNote
{
};

struct RwV3d
{
	f32 x;
	f32 y;
	f32 z;
};

struct UVAModelInfo
{
	xVec2 offset_vel;
	u32 flg_uvam;
	RpAtomic* model;
	RwTexCoords* uv;
	s32 uvsize;
	xVec2 offset;
};

struct zFragBone
{
	s32 index;
	xVec3 offset;
};

struct zFragLightningAsset : zFragAsset
{
	zFragLocation start;
	zFragLocation end;
	u32 startParentID;
	u32 endParentID;
};

struct xFactoryInst : RyzMemData
{
	s32 itemType;
	xFactoryInst* nextprod;
	xFactoryInst* prevprod;
};

struct rxHeapBlockHeader
{
	rxHeapBlockHeader* prev;
	rxHeapBlockHeader* next;
	u32 size;
	rxHeapFreeBlock* freeEntry;
	type_176 pad;
};

struct _tagPadAnalog
{
	s8 x;
	s8 y;
};

struct xParEmitterPropsAsset : xBaseAsset
{
	u32 parSysID;
	union
	{
		xParInterp rate;
		type_163 value;
	};
	xParInterp life;
	xParInterp size_birth;
	xParInterp size_death;
	type_200 color_birth;
	type_204 color_death;
	xParInterp vel_scale;
	xParInterp vel_angle;
	xVec3 vel;
	u32 emit_limit;
	f32 emit_limit_reset_time;
};

struct RpInterpolator
{
	s32 flags;
	s16 startMorphTarget;
	s16 endMorphTarget;
	f32 time;
	f32 recipTime;
	f32 position;
};

struct xCutsceneMgrAsset : xBaseAsset
{
	u32 cutsceneAssetID;
	u32 flags;
	f32 interpSpeed;
	type_174 startTime;
	type_182 endTime;
	type_188 emitID;
};

struct xAnimMultiFileBase
{
	u32 Count;
};

struct xJSPHeader
{
	type_4 idtag;
	u32 version;
	u32 jspNodeCount;
	RpClump* clump;
	xClumpCollBSPTree* colltree;
	xJSPNodeInfo* jspNodeList;
};

struct xPEVCyl
{
	f32 height;
	f32 radius;
	f32 deflection;
};

struct zScene : xScene
{
	_zPortal* pendingPortal;
	union
	{
		u32 num_ents;
		u32 num_base;
	};
	union
	{
		xBase** base;
		zEnt** ents;
	};
	u32 num_update_base;
	xBase** update_base;
	type_3 baseCount;
	type_12 baseList;
	_zEnv* zen;
};

struct zNPCB_SB2 : zNPCBoss
{
	_class_12 flag;
	s32 life;
	s32 round;
	s32 stage;
	f32 delay;
	f32 stage_delay;
	f32 player_damage_timer;
	u32 old_player_health;
	hand_enum active_hand;
	zNPCBPlankton* plankton;
	zNPCNewsFish* newsfish;
	u8 said_intro;
	type_79 models;
	type_85 nodes;
	type_93 hands;
	type_101 platforms;
	type_106 bounds;
	type_112 slugs;
	_class_4 turn;
	move_data move;
	_class_11 ymove;
	_class_0 sound_loc;
	f32 node_pulse;
	_class_6 glow_light;
};

struct xUpdateCullMgr
{
	u32 entCount;
	u32 entActive;
	void** ent;
	xUpdateCullEnt** mgr;
	u32 mgrCount;
	u32 mgrCurr;
	xUpdateCullEnt* mgrList;
	u32 grpCount;
	xUpdateCullGroup* grpList;
	type_32 activateCB;
	type_32 deactivateCB;
};

struct xAnimEffect
{
	xAnimEffect* Next;
	u32 Flags;
	f32 StartTime;
	f32 EndTime;
	type_68 Callback;
};

struct _anon0
{
};

enum en_ncinfx
{
	NCIN_FXTYP_UNKNOWN,
	NCIN_FXTYP_EXAMPLE,
	NCIN_FXTYP_BUBSLAM_LG,
	NCIN_FXTYP_BUBSLAM_SM,
	NCIN_FXTYP_SMOKETRAIL,
	NCIN_FXTYP_BUBTRAIL,
	NCIN_FXTYP_BUBTRAILBONE,
	NCIN_FXTYP_BUBWIPE,
	NCIN_FXTYP_JELLYLIGHT_01,
	NCIN_FXTYP_WATERSPLASH,
	NCIN_FXTYP_TARTARSHOOT,
	NCIN_FXTYP_OILSHOOT,
	NCIN_FXTYP_BONESHOOT,
	NCIN_FXTYP_BONETRAIL,
	NCIN_FXTYP_BOMBTRAIL,
	NCIN_FXTYP_OILHAZARD,
	NCIN_FXTYP_TARTARSTEAM,
	NCIN_FXTYP_ARFDOGBOOM,
	NCIN_FXTYP_SHIELDPOP,
	NCIN_FXTYP_TTGUNSMOKE,
	NCIN_FXTYP_SPATULAGLOW,
	NCIN_FXTYP_HOOKRECOIL,
	NCIN_FXTYP_FODDERPROD,
	NCIN_FXTYP_BZZTDANCE,
	NCIN_FXTYP_BZZTBEAM,
	NCIN_FXTYP_BADBREATH,
	NCIN_FXTYP_BOMBBLINK,
	NCIN_FXTYP_HAMSHOCK,
	NCIN_FXTYP_HAMSTREAK,
	NCIN_FXTYP_MONCLOUD,
	NCIN_FXTYP_MIDFISH,
	NCIN_FXTYP_GLOVEFRAG,
	NCIN_FXTYP_MARYBOOM,
	NCIN_FXTYP_PETEBONK,
	NCIN_FXTYP_FIRESPIRAL,
	NCIN_FXTYP_SLEEPYLAMP,
	NCIN_FXTYP_SLEEPYDRAY,
	NCIN_FXTYP_B101OPEN_SBENTER,
	NCIN_FXTYP_B101OPEN_PATENTER,
	NCIN_FXTYP_B101OPEN_SHOCKWAVE,
	NCIN_FXTYP_B101ROUND1_SHOCK1,
	NCIN_FXTYP_B101ROUND1_SHOCK2,
	NCIN_FXTYP_B101ROUND1_SHOCK3,
	NCIN_FXTYP_B101ENDING_SHOCK1,
	NCIN_FXTYP_B101ENDING_SHOCK2,
	NCIN_FXTYP_B101ENDING_SHOCK3,
	NCIN_FXTYP_B101ENDING_SHOCK4,
	NCIN_FXTYP_B101ENDING_SHOCKWAVE,
	NCIN_FXTYP_B201OPEN_FREEZE,
	NCIN_FXTYP_B201ROUND1_FREEZE,
	NCIN_FXTYP_B201HIDECONVEYOR,
	NCIN_FXTYP_B201GOOLEVER,
	NCIN_FXTYP_B201FRAG,
	NCIN_FXTYP_B303STUFF,
	NCIN_FXTYP_PAR_ENTITY_BONE,
	NCIN_FXTYP_PROBE,
	NCIN_FXTYP_DUPLOPUFFS,
	NCIN_FXTYP_SPAWNRING,
	NCIN_FXTYP_BUBPLYRSKEL,
	NCIN_FXTYP_GLINT,
	NCIN_FXTYP_NOMORE,
	NCIN_FXTYP_FORCE = 0x7fffffff
};

struct zFragAsset
{
	zFragType type;
	u32 id;
	type_205 parentID;
	f32 lifetime;
	f32 delay;
};

struct xEntMPData
{
	f32 curdist;
	f32 speed;
	xMovePoint* dest;
	xMovePoint* src;
	xSpline3* spl;
	f32 dist;
	u32 padalign;
	xQuat aquat;
	xQuat bquat;
};

struct xPar
{
	xPar* m_next;
	xPar* m_prev;
	f32 m_lifetime;
	type_134 m_c;
	xVec3 m_pos;
	f32 m_size;
	xVec3 m_vel;
	f32 m_sizeVel;
	u8 m_flag;
	u8 m_mode;
	type_153 m_texIdx;
	type_156 m_rotdeg;
	u8 pad8;
	f32 totalLifespan;
	xParEmitterAsset* m_asset;
	type_175 m_cvel;
	type_185 m_cfl;
};

struct zPlatform : zEnt
{
	xPlatformAsset* passet;
	xEntMotion motion;
	u16 state;
	u16 plat_flags;
	f32 tmr;
	s32 ctr;
	xMovePoint* src;
	xModelInstance* am;
	xModelInstance* bm;
	s32 moving;
	xEntDrive drv;
	zPlatFMRunTime* fmrt;
	f32 pauseMult;
	f32 pauseDelta;
};

struct NPCSndQueue
{
	u32 sndDirect;
	en_NPC_SOUND sndtype;
	s32 flg_snd;
	f32 tmr_delay;
	f32 radius;
};

struct NCINPar
{
	zParEmitter* emitter;
};

struct _class_2
{
	xModelTagWithNormal tag;
	xModelTag uptag;
};

struct xLinkAsset
{
	u16 srcEvent;
	u16 dstEvent;
	u32 dstAssetID;
	type_131 param;
	u32 paramWidgetAssetID;
	u32 chkAssetID;
};

struct _class_3
{
	f32 t;
	f32 u;
	f32 v;
};

struct xGroupAsset : xBaseAsset
{
	u16 itemCount;
	u16 groupFlags;
};

struct xParInterp
{
	type_105 val;
	u32 interp;
	f32 freq;
	f32 oofreq;
};

struct zNPCCommon : xNPCBasic
{
	xEntAsset* entass;
	xEntNPCAsset* npcass;
	zNPCSettings* npcsetass;
	s32 flg_vuln;
	s32 flg_move;
	s32 flg_misc;
	s32 flg_able;
	NPCConfig* cfg_npc;
	zNPCSettings npcset;
	zMovePoint* nav_past;
	zMovePoint* nav_curr;
	zMovePoint* nav_dest;
	zMovePoint* nav_lead;
	xSpline3* spl_mvptspline;
	f32 len_mvptspline;
	f32 dst_curspline;
	xEntDrive* drv_data;
	xPsyche* psy_instinct;
	zNPCCommon* npc_duplodude;
	f32 spd_throttle;
	s32 flg_xtrarend;
	f32 tmr_fidget;
	f32 tmr_invuln;
	zShrapnelAsset* explosion;
	xModelAssetParam* parmdata;
	u32 pdatsize;
	zNPCLassoInfo* lassdata;
	type_21 snd_queue;
};

struct RpPolygon
{
	u16 matIndex;
	type_110 vertIndex;
};

struct _tagiPad
{
	s32 port;
};

struct zShrapnelAsset
{
	s32 fassetCount;
	u32 shrapnelID;
	type_111 initCB;
};

struct zMovePoint : xMovePoint
{
};

struct xMovePoint : xBase
{
	xMovePointAsset* asset;
	xVec3* pos;
	xMovePoint** nodes;
	xMovePoint* prev;
	u32 node_wt_sum;
	u8 on;
	type_192 pad;
	f32 delay;
	xSpline3* spl;
};

struct xAnimPlay
{
	xAnimPlay* Next;
	u16 NumSingle;
	u16 BoneCount;
	xAnimSingle* Single;
	void* Object;
	xAnimTable* Table;
	xMemPool* Pool;
	xModelInstance* ModelInst;
	type_159 BeforeAnimMatrices;
};

struct NPARData
{
	xVec3 pos;
	type_115 xy_size;
	type_120 uv_tl;
	type_123 uv_br;
	RwRGBA color;
	f32 tmr_remain;
	f32 tym_exist;
	f32 fac_abuse;
	xVec3 vel;
	union
	{
		s32 flg_popts;
		s32 nparmode;
	};
	type_148 unused;
};

struct RpMaterialList
{
	RpMaterial** materials;
	s32 numMaterials;
	s32 space;
};

struct xParCmdAsset
{
	u32 type;
	u8 enabled;
	u8 mode;
	type_152 pad;
};

struct xVec4
{
	f32 x;
	f32 y;
	f32 z;
	f32 w;
};

union zFragLocInfo
{
	zFragBone bone;
	xModelTag tag;
};

struct tag_iFile
{
	u32 flags;
	type_118 path;
	s32 fd;
	s32 offset;
	s32 length;
};

struct xFFX
{
};

struct xModelPool
{
	xModelPool* Next;
	u32 NumMatrices;
	xModelInstance* List;
};

struct RwSurfaceProperties
{
	f32 ambient;
	f32 specular;
	f32 diffuse;
};

struct xSpline3
{
	u16 type;
	u16 flags;
	u32 N;
	u32 allocN;
	xVec3* points;
	f32* time;
	xVec3* p12;
	xVec3* bctrl;
	f32* knot;
	xCoef3* coef;
	u32 arcSample;
	f32* arcLength;
};

struct xAnimMultiFile : xAnimMultiFileBase
{
	type_119 Files;
};

struct RpMaterial
{
	RwTexture* texture;
	RwRGBA color;
	RxPipeline* pipeline;
	RwSurfaceProperties surfaceProps;
	s16 refCount;
	s16 pad;
};

struct xEntMotionERData
{
	xVec3 ret_pos;
	xVec3 ext_dpos;
	f32 ext_tm;
	f32 ext_wait_tm;
	f32 ret_tm;
	f32 ret_wait_tm;
};

struct xPEEntBone
{
	u8 flags;
	u8 type;
	u8 bone;
	u8 pad1;
	xVec3 offset;
	f32 radius;
	f32 deflection;
};

struct rxReq
{
};

struct zFragSound
{
	zFragSoundAsset* fasset;
	xVec3 location;
	u32 soundID;
};

struct _class_4
{
	xVec2 dir;
	f32 vel;
	f32 accel;
	f32 max_vel;
};

struct _class_5
{
	type_122 base_point;
	type_126 point;
	s16 total_points;
	s16 end_points;
	f32 arc_height;
	xVec3 arc_normal;
	type_138 thickness;
	union
	{
		_tagLightningLine line;
		_tagLightningRot rot;
		_tagLightningZeus zeus;
	};
	f32 rand_radius;
};

struct _class_6
{
	xLightKit kit;
	type_124 light;
};

struct tag_xFile
{
	type_145 relname;
	tag_iFile ps;
	void* user_data;
};

struct xPsyche : RyzMemData
{
	xBase* clt_owner;
	xPSYNote* cb_notice;
	s32 flg_psyche;
	xGoal* goallist;
	type_212 goalstak;
	type_216 tmr_stack;
	s32 staktop;
	xGoal* pendgoal;
	en_pendtype pendtype;
	s32 gid_safegoal;
	type_2 fun_remap;
	void* userContext;
	s32 cnt_transLastTimestep;
	PSY_BRAIN_STATUS psystat;
	xBase fakebase;
};

struct HAZCatProd : HAZBall
{
	zLightning* zap_lyta;
	zLightning* zap_lytb;
};

struct NCINEnts
{
	type_125 ent;
};

struct zFragSoundAsset : zFragAsset
{
	u32 assetID;
	zFragLocation source;
	f32 volume;
	f32 innerRadius;
	f32 outerRadius;
};

enum en_NPC_SOUND
{
	NPC_STYP_BOGUS = 0xfffffffe,
	NPC_STYP_LISTEND = 0,
	NPC_STYP_ENCOUNTER,
	NPC_STYP_CLANKING,
	NPC_STYP_EXCLAIM,
	NPC_STYP_OUCH,
	NPC_STYP_CHEERING,
	NPC_STYP_RESPAWN,
	NPC_STYP_ALERT,
	NPC_STYP_DIZZY,
	NPC_STYP_DANCE,
	NPC_STYP_LAUGH,
	NPC_STYP_ATTACK,
	NPC_STYP_PUNCH,
	NPC_STYP_WEPLAUNCH,
	NPC_STYP_LIGHTNING,
	NPC_STYP_WARNBANG,
	NPC_STYP_DEATH,
	NPC_STYP_DEATHJELLY,
	NPC_STYP_BONKED,
	NPC_STYP_UNBONKED,
	NPC_STYP_TIKISTACK,
	NPC_STYP_TIKIEXPLODE,
	NPC_STYP_TIKITHUNDER,
	NPC_STYP_XSFXTALK,
	NPC_STYP_ONELINER,
	NPC_STYP_ONELINERTOO,
	NPC_STYP_NOMORE,
	NPC_STYP_FORCE = 0x7fffffff
};

enum RxClusterValidityReq
{
	rxCLREQ_DONTWANT,
	rxCLREQ_REQUIRED,
	rxCLREQ_OPTIONAL,
	rxCLUSTERVALIDITYREQFORCEENUMSIZEINT = 0x7fffffff
};

struct xEntNPCAsset
{
	s32 npcFlags;
	s32 npcModel;
	s32 npcProps;
	u32 movepoint;
	u32 taskWidgetPrime;
	u32 taskWidgetSecond;
};

enum slug_stage
{
	SLUG_INACTIVE,
	SLUG_AIM,
	SLUG_DELAY,
	SLUG_DYING,
	SLUG_FIRE
};

struct _class_7
{
	type_127 tag;
};

struct xEntShadow
{
	xVec3 pos;
	xVec3 vec;
	RpAtomic* shadowModel;
	f32 dst_cast;
	type_209 radius;
};

struct RpWorldSector
{
	s32 type;
	RpPolygon* polygons;
	RwV3d* vertices;
	RpVertexNormal* normals;
	type_147 texCoords;
	RwRGBA* preLitLum;
	RwResEntry* repEntry;
	RwLinkList collAtomicsInWorldSector;
	RwLinkList noCollAtomicsInWorldSector;
	RwLinkList lightsInWorldSector;
	RwBBox boundingBox;
	RwBBox tightBoundingBox;
	RpMeshHeader* mesh;
	RxPipeline* pipeline;
	u16 matListWindowBase;
	u16 numVertices;
	u16 numPolygons;
	u16 pad;
};

struct xSurface : xBase
{
	u32 idx;
	u32 type;
	union
	{
		u32 mat_idx;
		xEnt* ent;
		void* obj;
	};
	f32 friction;
	u8 state;
	type_207 pad;
	void* moprops;
};

struct zJumpParam
{
	f32 PeakHeight;
	f32 TimeGravChange;
	f32 TimeHold;
	f32 ImpulseVel;
};

struct zGlobals : xGlobals
{
	zPlayerGlobals player;
	zAssetPickupTable* pickupTable;
	zCutsceneMgr* cmgr;
	zScene* sceneCur;
	zScene* scenePreload;
};

struct anim_coll_data
{
};

struct zFragLocation
{
	zFragLocType type;
	zFragLocInfo info;
};

enum RxNodeDefEditable
{
	rxNODEDEFCONST,
	rxNODEDEFEDITABLE,
	rxNODEDEFEDITABLEFORCEENUMSIZEINT = 0x7fffffff
};

struct xDynAsset : xBaseAsset
{
	u32 type;
	u16 version;
	u16 handle;
};

struct RpMorphTarget
{
	RpGeometry* parentGeom;
	RwSphere boundingSphere;
	RwV3d* verts;
	RwV3d* normals;
};

enum RxClusterValid
{
	rxCLVALID_NOCHANGE,
	rxCLVALID_VALID,
	rxCLVALID_INVALID,
	rxCLUSTERVALIDFORCEENUMSIZEINT = 0x7fffffff
};

struct xQCData
{
	s8 xmin;
	s8 ymin;
	s8 zmin;
	s8 zmin_dup;
	s8 xmax;
	s8 ymax;
	s8 zmax;
	s8 zmax_dup;
	xVec3 min;
	xVec3 max;
};

struct zGlobalSettings
{
	u16 AnalogMin;
	u16 AnalogMax;
	f32 SundaeTime;
	f32 SundaeMult;
	u32 InitialShinyCount;
	u32 InitialSpatulaCount;
	s32 ShinyValuePurple;
	s32 ShinyValueBlue;
	s32 ShinyValueGreen;
	s32 ShinyValueYellow;
	s32 ShinyValueRed;
	s32 ShinyValueCombo0;
	s32 ShinyValueCombo1;
	s32 ShinyValueCombo2;
	s32 ShinyValueCombo3;
	s32 ShinyValueCombo4;
	s32 ShinyValueCombo5;
	s32 ShinyValueCombo6;
	s32 ShinyValueCombo7;
	s32 ShinyValueCombo8;
	s32 ShinyValueCombo9;
	s32 ShinyValueCombo10;
	s32 ShinyValueCombo11;
	s32 ShinyValueCombo12;
	s32 ShinyValueCombo13;
	s32 ShinyValueCombo14;
	s32 ShinyValueCombo15;
	f32 ComboTimer;
	u32 Initial_Specials;
	u32 TakeDamage;
	f32 DamageTimeHit;
	f32 DamageTimeSurface;
	f32 DamageTimeEGen;
	f32 DamageSurfKnock;
	f32 DamageGiveHealthKnock;
	u32 CheatSpongeball;
	u32 CheatPlayerSwitch;
	u32 CheatAlwaysPortal;
	u32 CheatFlyToggle;
	u32 DisableForceConversation;
	f32 StartSlideAngle;
	f32 StopSlideAngle;
	f32 RotMatchMaxAngle;
	f32 RotMatchMatchTime;
	f32 RotMatchRelaxTime;
	f32 Gravity;
	f32 BBashTime;
	f32 BBashHeight;
	f32 BBashDelay;
	f32 BBashCVTime;
	f32 BBounceSpeed;
	f32 BSpinMinFrame;
	f32 BSpinMaxFrame;
	f32 BSpinRadius;
	f32 SandyMeleeMinFrame;
	f32 SandyMeleeMaxFrame;
	f32 SandyMeleeRadius;
	f32 BubbleBowlTimeDelay;
	f32 BubbleBowlLaunchPosLeft;
	f32 BubbleBowlLaunchPosUp;
	f32 BubbleBowlLaunchPosAt;
	f32 BubbleBowlLaunchVelLeft;
	f32 BubbleBowlLaunchVelUp;
	f32 BubbleBowlLaunchVelAt;
	f32 BubbleBowlPercentIncrease;
	f32 BubbleBowlMinSpeed;
	f32 BubbleBowlMinRecoverTime;
	f32 SlideAccelVelMin;
	f32 SlideAccelVelMax;
	f32 SlideAccelStart;
	f32 SlideAccelEnd;
	f32 SlideAccelPlayerFwd;
	f32 SlideAccelPlayerBack;
	f32 SlideAccelPlayerSide;
	f32 SlideVelMaxStart;
	f32 SlideVelMaxEnd;
	f32 SlideVelMaxIncTime;
	f32 SlideVelMaxIncAccel;
	f32 SlideAirHoldTime;
	f32 SlideAirSlowTime;
	f32 SlideAirDblHoldTime;
	f32 SlideAirDblSlowTime;
	f32 SlideVelDblBoost;
	u8 SlideApplyPhysics;
	type_8 PowerUp;
	type_16 InitialPowerUp;
};

struct xPECircle
{
	f32 radius;
	f32 deflection;
	xVec3 dir;
};

struct xRot
{
	xVec3 axis;
	f32 angle;
};

struct zAssetPickupTable
{
};

struct NCINMat
{
	RwMatrixTag* mat;
};

struct xClumpCollBSPVertInfo
{
	u16 atomIndex;
	u16 meshVertIndex;
};

enum hand_enum
{
	LEFT_HAND,
	RIGHT_HAND
};

struct NCINShrap
{
	zShrapnelAsset* shrap;
};

struct xClumpCollBSPBranchNode
{
	u32 leftInfo;
	u32 rightInfo;
	f32 leftValue;
	f32 rightValue;
};

struct zFragShockwave
{
	zFragShockwaveAsset* fasset;
	f32 currSize;
	f32 currVelocity;
	f32 deltVelocity;
	f32 currSpin;
	f32 deltSpin;
	type_167 currColor;
	type_173 deltColor;
};

struct move_config
{
	f32 accel;
	f32 max_vel;
	f32 turn_accel;
	f32 turn_max_vel;
};

struct _tagLightningLine
{
	f32 unused;
};

struct slug_data
{
	slug_stage stage;
	u8 spun;
	u8 abandoned;
	f32 time;
	f32 stage_delay;
	xEnt* ent;
	platform_data* target;
	xMat4x3 mat;
	xMat4x3 dmat;
	xVec3 move_dir;
	f32 dist;
	f32 vel;
	f32 accel;
	f32 end_dist;
	f32 max_vel;
	f32 ydist;
	f32 yvel;
	f32 end_ydist;
	u32 sound_handle;
};

struct zPlayerLassoInfo
{
	xEnt* target;
	f32 dist;
	u8 destroy;
	u8 targetGuide;
	f32 lassoRot;
	xEnt* swingTarget;
	xEnt* releasedSwing;
	f32 copterTime;
	s32 canCopter;
	zLasso lasso;
	xAnimState* zeroAnim;
};

struct HAZMissile
{
	f32 vel;
};

enum rxEmbeddedPacketState
{
	rxPKST_PACKETLESS,
	rxPKST_UNUSED,
	rxPKST_INUSE,
	rxPKST_PENDING,
	rxEMBEDDEDPACKETSTATEFORCEENUMSIZEINT = 0x7fffffff
};

struct xSphere
{
	xVec3 center;
	f32 r;
};

struct xClumpCollBSPTriangle
{
	_class_9 v;
	u8 flags;
	u8 platData;
	u16 matIndex;
};

union NCINData
{
	NCINLyt lytdata;
	NCINHaz hazdata;
	NCINLamp lampdata;
	NCINDRay draydata;
	NCINStrk strkdata;
	NCINArc arcdata;
	NCINPar pardata;
	NCINEnts entdata;
	NCINShrap shrapdata;
	NCINCustom customdata;
	NCINRast rastinfo;
	NCINMat matdata;
};

struct NPCCone
{
	f32 rad_cone;
	RwRGBA rgba_top;
	RwRGBA rgba_bot;
	RwRaster* rast_cone;
	type_168 uv_tip;
	type_177 uv_slice;
};

struct xEntMotionOrbitData
{
	xVec3 center;
	f32 w;
	f32 h;
	f32 period;
};

struct xGroup : xBase
{
	xGroupAsset* asset;
	xBase** item;
	u32 last_index;
	s32 flg_group;
};

struct zLedgeGrabParams
{
	f32 animGrab;
	f32 zdist;
	type_161 tranTable;
	s32 tranCount;
	xEnt* optr;
	xMat4x3 omat;
	f32 y0det;
	f32 dydet;
	f32 r0det;
	f32 drdet;
	f32 thdet;
	f32 rtime;
	f32 ttime;
	f32 tmr;
	xVec3 spos;
	xVec3 epos;
	xVec3 tpos;
	s32 nrays;
	s32 rrand;
	f32 startrot;
	f32 endrot;
};

enum RwCameraProjection
{
	rwNACAMERAPROJECTION,
	rwPERSPECTIVE,
	rwPARALLEL,
	rwCAMERAPROJECTIONFORCEENUMSIZEINT = 0x7fffffff
};

struct RpMeshHeader
{
	u32 flags;
	u16 numMeshes;
	u16 serialNum;
	u32 totalIndicesInMesh;
	u32 firstMeshOffset;
};

enum RxClusterForcePresent
{
	rxCLALLOWABSENT,
	rxCLFORCEPRESENT,
	rxCLUSTERFORCEPRESENTFORCEENUMSIZEINT = 0x7fffffff
};

enum zFragType
{
	eFragInactive,
	eFragGroup,
	eFragShrapnel,
	eFragParticle,
	eFragProjectile,
	eFragLightning,
	eFragSound,
	eFragShockwave,
	eFragCount,
	eFragForceSize = 0x7fffffff
};

struct xCylinder
{
	xVec3 center;
	f32 r;
	f32 h;
};

struct xCutsceneInfo
{
	u32 Magic;
	u32 AssetID;
	u32 NumData;
	u32 NumTime;
	u32 MaxModel;
	u32 MaxBufEven;
	u32 MaxBufOdd;
	u32 HeaderSize;
	u32 VisCount;
	u32 VisSize;
	u32 BreakCount;
	u32 pad;
	type_201 SoundLeft;
	type_206 SoundRight;
};

struct xMemPool
{
	void* FreeList;
	u16 NextOffset;
	u16 Flags;
	void* UsedList;
	type_11 InitCB;
	void* Buffer;
	u16 Size;
	u16 NumRealloc;
	u32 Total;
};

union RxColorUnion
{
	RwRGBA preLitColor;
	RwRGBA color;
};

union _class_8
{
	HAZTypical typical;
	HAZCollide collide;
	HAZBall ball;
	HAZRing ring;
	HAZShroom shroom;
	HAZCloud cloud;
	HAZPatriot patriot;
	HAZTarTar tartar;
	HAZCatProd catprod;
};

struct xBox
{
	xVec3 upper;
	xVec3 lower;
};

enum _zPlayerWallJumpState
{
	k_WALLJUMP_NOT,
	k_WALLJUMP_LAUNCH,
	k_WALLJUMP_FLIGHT,
	k_WALLJUMP_LAND
};

enum en_pendtype
{
	PEND_TRAN_NONE,
	PEND_TRAN_SET,
	PEND_TRAN_PUSH,
	PEND_TRAN_POP,
	PEND_TRAN_POPTO,
	PEND_TRAN_POPALL,
	PEND_TRAN_SWAP,
	PEND_TRAN_INPROG,
	PEND_TRAN_NOMORE
};

struct NCINLyt
{
	zLightning* lyt_zap;
};

struct RxClusterDefinition
{
	s8* name;
	u32 defaultStride;
	u32 defaultAttributes;
	s8* attributeSet;
};

struct NCINCustom
{
	type_165 f;
	type_170 v;
};

struct RyzMemData
{
};

struct _tagEmitSphere
{
	f32 radius;
};

struct platform_data
{
	u8 stopping;
	xEnt* ent;
	f32 radius;
	xMat3x3 mat;
	_class_13 spin;
};

struct HAZPatriot : HAZMissile
{
	xVec3 pos_began;
	f32 spd_peak;
	f32 spd_curr;
	f32 acc_rate;
};

struct RpTriangle
{
	type_171 vertIndex;
	s16 matIndex;
};

struct _tagLightningRot
{
	type_179 deg;
	f32 degrees;
	f32 height;
};

struct move_data : move_config
{
	xVec2 dest;
	xVec2 dir;
	f32 vel;
	f32 yaw;
	f32 yaw_vel;
};

struct hand_data
{
	u8 hurt_player;
	u8 hit_platforms;
	type_190 head_tag;
	type_194 tail_tag;
	xEnt* ent;
	f32 radius;
};

enum _tagRumbleType
{
	eRumble_Off,
	eRumble_Hi,
	eRumble_VeryLightHi,
	eRumble_VeryLight,
	eRumble_LightHi,
	eRumble_Light,
	eRumble_MediumHi,
	eRumble_Medium,
	eRumble_HeavyHi,
	eRumble_Heavy,
	eRumble_VeryHeavyHi,
	eRumble_VeryHeavy,
	eRumble_Total,
	eRumbleForceU32 = 0x7fffffff
};

struct RwSphere
{
	RwV3d center;
	f32 radius;
};

struct xCutsceneTime
{
	f32 StartTime;
	f32 EndTime;
	u32 NumData;
	u32 ChunkIndex;
};

enum _zPlayerType
{
	ePlayer_SB,
	ePlayer_Patrick,
	ePlayer_Sandy,
	ePlayer_MAXTYPES
};

struct xEntMechData
{
	xVec3 apos;
	xVec3 bpos;
	xVec3 dir;
	f32 arot;
	f32 brot;
	f32 ss;
	f32 sr;
	s32 state;
	f32 tsfd;
	f32 trfd;
	f32 tsbd;
	f32 trbd;
	f32* rotptr;
};

union _class_9
{
	xClumpCollBSPVertInfo i;
	RwV3d* p;
};

struct xEntMotionSplineData
{
	s32 unknown;
};

struct xListItem_0
{
	s32 flg_travFilter;
	xGoal* next;
	xGoal* prev;
};

struct HAZTypical
{
	xMat3x3 mat_rotDelta;
	f32 rad_min;
	f32 rad_max;
	f32 rad_cur;
};

struct RwTexDictionary
{
	RwObject object;
	RwLinkList texturesInDict;
	RwLLLink lInInstance;
};

struct xCutsceneData
{
	u32 DataType;
	u32 AssetID;
	u32 ChunkSize;
	union
	{
		u32 FileOffset;
		void* DataPtr;
	};
};

struct RxOutputSpec
{
	s8* name;
	RxClusterValid* outputClusters;
	RxClusterValid allOtherClusters;
};

struct analog_data
{
	xVec2 offset;
	xVec2 dir;
	f32 mag;
	f32 ang;
};

struct NCINHaz
{
	NPCHazard* npchaz;
};

struct zFragGroup
{
	type_197 list;
};

enum en_hazcol
{
	HAZ_COLTYP_STAT,
	HAZ_COLTYP_DYN,
	HAZ_COLTYP_NPC,
	HAZ_COLTYP_NOMORE,
	HAZ_COLTYP_FORCE = 0x7fffffff
};

struct NCINRast
{
	RwRaster* raster;
};

struct _tagEmitRect
{
	f32 x_len;
	f32 z_len;
};

struct zLasso
{
	u32 flags;
	f32 secsTotal;
	f32 secsLeft;
	f32 stRadius;
	f32 tgRadius;
	f32 crRadius;
	xVec3 stCenter;
	xVec3 tgCenter;
	xVec3 crCenter;
	xVec3 stNormal;
	xVec3 tgNormal;
	xVec3 crNormal;
	xVec3 honda;
	f32 stSlack;
	f32 stSlackDist;
	f32 tgSlack;
	f32 tgSlackDist;
	f32 crSlack;
	f32 currDist;
	f32 lastDist;
	type_169 lastRefs;
	type_180 reindex;
	xVec3 anchor;
	xModelTag tag;
	xModelInstance* model;
};

struct xMat3x3
{
	xVec3 right;
	s32 flags;
	xVec3 up;
	u32 pad1;
	xVec3 at;
	u32 pad2;
};

struct _class_10
{
	type_199 endPoint;
	xVec3 direction;
	f32 length;
	f32 scale;
	f32 width;
	type_211 endParam;
	type_214 endVel;
	type_218 paramSpan;
	f32 arc_height;
	xVec3 arc_normal;
};

enum en_nparptyp
{
	NPAR_TYP_UNKNOWN,
	NPAR_TYP_OILBUB,
	NPAR_TYP_TUBESPIRAL,
	NPAR_TYP_TUBECONFETTI,
	NPAR_TYP_GLOVEDUST,
	NPAR_TYP_MONSOONRAIN,
	NPAR_TYP_SLEEPYZEEZ,
	NPAR_TYP_CHUCKSPLASH,
	NPAR_TYP_TARTARGUNK,
	NPAR_TYP_DOGBREATH,
	NPAR_TYP_VISSPLASH,
	NPAR_TYP_FIREWORKS,
	NPAR_TYP_NOMORE,
	NPAR_TYP_FORCE = 0x7fffffff
};

union zFragInfo
{
	zFragGroup group;
	zFragParticle particle;
	zFragProjectile projectile;
	zFragLightning lightning;
	zFragSound sound;
	zFragShockwave shockwave;
};

struct RxClusterRef
{
	RxClusterDefinition* clusterDef;
	RxClusterForcePresent forcePresent;
	u32 reserved;
};

struct xListItem_1
{
	s32 flg_travFilter;
	NPCConfig* next;
	NPCConfig* prev;
};

struct xModelTag
{
	xVec3 v;
	u32 matidx;
	type_220 wt;
};

struct RwObject
{
	u8 type;
	u8 subType;
	u8 flags;
	u8 privateFlags;
	void* parent;
};

struct zCheckPoint
{
	xVec3 pos;
	f32 rot;
	u32 initCamID;
};

struct HAZCollide : HAZTypical
{
	xVec3 pos_collide;
	xVec3 dir_normal;
	xParabola parabinfo;
	union
	{
		s32 flg_collide;
		s32 flg_result;
		s32 flg_unused;
	};
	s32 cnt_skipcol;
	en_hazcol idx_rotateCol;
};

struct xEntMotionMPData
{
	u32 flags;
	u32 mp_id;
	f32 speed;
};

struct xEntERData
{
	xVec3 a;
	xVec3 b;
	xVec3 dir;
	f32 et;
	f32 wet;
	f32 rt;
	f32 wrt;
	f32 p;
	f32 brt;
	f32 ert;
	s32 state;
};

struct zPlayerGlobals
{
	zEnt ent;
	xEntShadow entShadow_embedded;
	xShadowSimpleCache simpShadow_embedded;
	zGlobalSettings g;
	zPlayerSettings* s;
	zPlayerSettings sb;
	zPlayerSettings patrick;
	zPlayerSettings sandy;
	xModelInstance* model_spongebob;
	xModelInstance* model_patrick;
	xModelInstance* model_sandy;
	u32 Visible;
	u32 Health;
	s32 Speed;
	f32 SpeedMult;
	s32 Sneak;
	s32 Teeter;
	f32 SlipFadeTimer;
	s32 Slide;
	f32 SlideTimer;
	s32 Stepping;
	s32 JumpState;
	s32 LastJumpState;
	f32 JumpTimer;
	f32 LookAroundTimer;
	u32 LookAroundRand;
	u32 LastProjectile;
	f32 DecelRun;
	f32 DecelRunSpeed;
	f32 HotsauceTimer;
	f32 LeanLerp;
	f32 ScareTimer;
	xBase* ScareSource;
	f32 CowerTimer;
	f32 DamageTimer;
	f32 SundaeTimer;
	f32 ControlOffTimer;
	f32 HelmetTimer;
	u32 WorldDisguise;
	u32 Bounced;
	f32 FallDeathTimer;
	f32 HeadbuttVel;
	f32 HeadbuttTimer;
	u32 SpecialReceived;
	xEnt* MountChimney;
	f32 MountChimOldY;
	u32 MaxHealth;
	u32 DoMeleeCheck;
	f32 VictoryTimer;
	f32 BadGuyNearTimer;
	f32 ForceSlipperyTimer;
	f32 ForceSlipperyFriction;
	f32 ShockRadius;
	f32 ShockRadiusOld;
	f32 Face_ScareTimer;
	u32 Face_ScareRandom;
	u32 Face_Event;
	f32 Face_EventTimer;
	f32 Face_PantTimer;
	u32 Face_AnimSpecific;
	u32 IdleRand;
	f32 IdleMinorTimer;
	f32 IdleMajorTimer;
	f32 IdleSitTimer;
	s32 Transparent;
	zEnt* FireTarget;
	u32 ControlOff;
	u32 ControlOnEvent;
	u32 AutoMoveSpeed;
	f32 AutoMoveDist;
	xVec3 AutoMoveTarget;
	xBase* AutoMoveObject;
	zEnt* Diggable;
	f32 DigTimer;
	zPlayerCarryInfo carry;
	zPlayerLassoInfo lassoInfo;
	type_30 BubbleWandTag;
	xModelInstance* model_wand;
	xEntBoulder* bubblebowl;
	f32 bbowlInitVel;
	zEntHangable* HangFound;
	zEntHangable* HangEnt;
	zEntHangable* HangEntLast;
	xVec3 HangPivot;
	xVec3 HangVel;
	f32 HangLength;
	xVec3 HangStartPos;
	f32 HangStartLerp;
	type_90 HangPawTag;
	f32 HangPawOffset;
	f32 HangElapsed;
	f32 Jump_CurrGravity;
	f32 Jump_HoldTimer;
	f32 Jump_ChangeTimer;
	s32 Jump_CanDouble;
	s32 Jump_CanFloat;
	s32 Jump_SpringboardStart;
	zPlatform* Jump_Springboard;
	s32 CanJump;
	s32 CanBubbleSpin;
	s32 CanBubbleBounce;
	s32 CanBubbleBash;
	s32 IsJumping;
	s32 IsDJumping;
	s32 IsBubbleSpinning;
	s32 IsBubbleBouncing;
	s32 IsBubbleBashing;
	s32 IsBubbleBowling;
	s32 WasDJumping;
	s32 IsCoptering;
	_zPlayerWallJumpState WallJumpState;
	s32 cheat_mode;
	u32 Inv_Shiny;
	u32 Inv_Spatula;
	type_193 Inv_PatsSock;
	type_196 Inv_PatsSock_Max;
	u32 Inv_PatsSock_CurrentLevel;
	type_203 Inv_LevelPickups;
	u32 Inv_LevelPickups_CurrentLevel;
	u32 Inv_PatsSock_Total;
	xModelTag BubbleTag;
	xEntDrive drv;
	xSurface* floor_surf;
	xVec3 floor_norm;
	s32 slope;
	xCollis earc_coll;
	xSphere head_sph;
	xModelTag center_tag;
	xModelTag head_tag;
	type_10 TongueFlags;
	xVec3 RootUp;
	xVec3 RootUpTarget;
	zCheckPoint cp;
	u32 SlideTrackSliding;
	u32 SlideTrackCount;
	type_48 SlideTrackEnt;
	u32 SlideNotGroundedSinceSlide;
	xVec3 SlideTrackDir;
	xVec3 SlideTrackVel;
	f32 SlideTrackDecay;
	f32 SlideTrackLean;
	f32 SlideTrackLand;
	type_88 sb_model_indices;
	type_99 sb_models;
	u32 currentPlayer;
	xVec3 PredictRotate;
	xVec3 PredictTranslate;
	f32 PredictAngV;
	xVec3 PredictCurrDir;
	f32 PredictCurrVel;
	f32 KnockBackTimer;
	f32 KnockIntoAirTimer;
};

struct _tagLightningZeus
{
	f32 normal_offset;
	f32 back_offset;
	f32 side_offset;
};

struct RxIoSpec
{
	u32 numClustersOfInterest;
	RxClusterRef* clustersOfInterest;
	RxClusterValidityReq* inputRequirements;
	u32 numOutputs;
	RxOutputSpec* outputs;
};

struct xBaseAsset
{
	u32 id;
	u8 baseType;
	u8 linkCount;
	u16 baseFlags;
};

enum RpWorldRenderOrder
{
	rpWORLDRENDERNARENDERORDER,
	rpWORLDRENDERFRONT2BACK,
	rpWORLDRENDERBACK2FRONT,
	rpWORLDRENDERORDERFORCEENUMSIZEINT = 0x7fffffff
};

struct _class_11
{
	f32 begin;
	f32 end;
	f32 time;
	f32 end_time;
};

struct xEntDrive
{
	u32 flags;
	f32 otm;
	f32 otmr;
	f32 os;
	f32 tm;
	f32 tmr;
	f32 s;
	xEnt* odriver;
	xEnt* driver;
	xEnt* driven;
	xVec3 op;
	xVec3 p;
	xVec3 q;
	f32 yaw;
	xVec3 dloc;
	tri_data_0 tri;
};

struct xParabola
{
	xVec3 initPos;
	xVec3 initVel;
	f32 gravity;
	f32 minTime;
	f32 maxTime;
};

struct xShadowPoly
{
	type_213 vert;
	xVec3 norm;
};

struct RxNodeMethods
{
	type_0 nodeBody;
	type_9 nodeInit;
	type_22 nodeTerm;
	type_33 pipelineNodeInit;
	type_42 pipelineNodeTerm;
	type_54 pipelineNodeConfig;
	type_76 configMsgHandler;
};

struct zPlayerCarryInfo
{
	xEnt* grabbed;
	u32 grabbedModelID;
	xMat4x3 spin;
	xEnt* throwTarget;
	xEnt* flyingToTarget;
	f32 minDist;
	f32 maxDist;
	f32 minHeight;
	f32 maxHeight;
	f32 maxCosAngle;
	f32 throwMinDist;
	f32 throwMaxDist;
	f32 throwMinHeight;
	f32 throwMaxHeight;
	f32 throwMaxStack;
	f32 throwMaxCosAngle;
	f32 throwTargetRotRate;
	f32 targetRot;
	u32 grabTarget;
	xVec3 grabOffset;
	f32 grabLerpMin;
	f32 grabLerpMax;
	f32 grabLerpLast;
	u32 grabYclear;
	f32 throwGravity;
	f32 throwHeight;
	f32 throwDistance;
	f32 fruitFloorDecayMin;
	f32 fruitFloorDecayMax;
	f32 fruitFloorBounce;
	f32 fruitFloorFriction;
	f32 fruitCeilingBounce;
	f32 fruitWallBounce;
	f32 fruitLifetime;
	xEnt* patLauncher;
};

struct zFragParticle
{
	zFragParticleAsset* fasset;
};

struct HAZBall : HAZTypical
{
};

struct _class_12
{
	u8 face_player;
	u8 face_follow;
	u8 dizzy;
	move_enum move;
	u8 vulnerable;
	u8 cruise_exploding;
	u8 cruise_hit_body;
	u8 cruise_hit_target;
	u8 nodes_taken;
	u8 updated;
};

struct xPEEntBound
{
	u8 flags;
	u8 type;
	u8 pad1;
	u8 pad2;
	f32 expand;
	f32 deflection;
};

enum _SDRenderState
{
	SDRS_Unknown,
	SDRS_Default,
	SDRS_OpaqueModels,
	SDRS_AlphaModels,
	SDRS_Bubble,
	SDRS_Projectile,
	SDRS_Font,
	SDRS_HUD,
	SDRS_Particles,
	SDRS_Lightning,
	SDRS_Streak,
	SDRS_SkyBack,
	SDRS_Environment,
	SDRS_Fill,
	SDRS_NPCVisual,
	SDRS_OOBFade,
	SDRS_OOBPlayerZ,
	SDRS_OOBPlayerAlpha,
	SDRS_OOBHand,
	SDRS_Glare,
	SDRS_Newsfish,
	SDRS_CruiseHUD,
	SDRS_DiscoFloorGlow,
	SDRS_Total = 0xffffffff
};

enum PSY_BRAIN_STATUS
{
	PSY_STAT_BLANK,
	PSY_STAT_GROW,
	PSY_STAT_EXTEND,
	PSY_STAT_THINK,
	PSY_STAT_NOMORE,
	PSY_STAT_FORCE = 0x7fffffff
};

struct xVec2
{
	f32 x;
	f32 y;
};

struct _tagEmitLine
{
	xVec3 pos1;
	xVec3 pos2;
	f32 radius;
};

enum en_npcbtyp
{
	NPCP_BASIS_NONE,
	NPCP_BASIS_EVILROBOT,
	NPCP_BASIS_FRIENDLYROBOT,
	NPCP_BASIS_LOVINGCITIZEN,
	NPCP_BASIS_GRUMPYCITIZEN,
	NPCP_BASIS_NOMORE,
	NPCP_BASIS_FORCE = 0x7fffffff
};

struct xModelBucket
{
	RpAtomic* Data;
	RpAtomic* OriginalData;
	xModelInstance* List;
	s32 ClipFlags;
	u32 PipeFlags;
};

struct RwFrustumPlane
{
	RwPlane plane;
	u8 closestX;
	u8 closestY;
	u8 closestZ;
	u8 pad;
};

struct xCoef
{
	type_222 a;
};

enum en_npctgt
{
	NPC_TGT_NONE,
	NPC_TGT_PLYR,
	NPC_TGT_ENT,
	NPC_TGT_BASE,
	NPC_TGT_POS,
	NPC_TGT_MVPT,
	NPC_TGT_NOMORE,
	NPC_TGT_FORCEINT = 0x7fffffff
};

struct RwPlane
{
	RwV3d normal;
	f32 distance;
};

struct xBBox
{
	xVec3 center;
	xBox box;
};

struct xEntMotionMechData
{
	u8 type;
	u8 flags;
	u8 sld_axis;
	u8 rot_axis;
	f32 sld_dist;
	f32 sld_tm;
	f32 sld_acc_tm;
	f32 sld_dec_tm;
	f32 rot_dist;
	f32 rot_tm;
	f32 rot_acc_tm;
	f32 rot_dec_tm;
	f32 ret_delay;
	f32 post_ret_delay;
};

enum en_GOALSTATE
{
	GOAL_STAT_UNKNOWN,
	GOAL_STAT_PROCESS,
	GOAL_STAT_ENTER,
	GOAL_STAT_EXIT,
	GOAL_STAT_SUSPEND,
	GOAL_STAT_RESUME,
	GOAL_STAT_PAUSED,
	GOAL_STAT_DONE,
	GOAL_STAT_NOMORE,
	GOAL_STAT_FORCE = 0x7fffffff
};

struct xNPCBasic : xEnt, xFactoryInst
{
	type_1 f_setup;
	type_7 f_reset;
	union
	{
		s32 flg_basenpc;
		s32 inUpdate;
		s32 flg_upward;
	};
	s32 colFreq;
	s32 colFreqReset;
	union
	{
		u32 flg_colCheck;
		u32 flg_penCheck;
		u32 flg_unused;
	};
	s32 myNPCType;
	xEntShadow entShadow_embedded;
	xShadowSimpleCache simpShadow_embedded;
};

enum move_enum
{
	MOVE_NONE,
	MOVE_HALT,
	MOVE_FOLLOW,
	MOVE_Y
};

struct _class_13
{
	xVec3 axis;
	f32 ang;
	f32 end_ang;
	f32 vel;
	f32 max_vel;
	f32 accel;
};

struct RxCluster
{
	u16 flags;
	u16 stride;
	void* data;
	void* currentData;
	u32 numAlloced;
	u32 numUsed;
	RxPipelineCluster* clusterRef;
	u32 attributes;
};

struct zNPCBoss : zNPCCommon
{
	NPCTarget tgt_cur;
};

struct xParEmitterCustomSettings : xParEmitterPropsAsset
{
	u32 custom_flags;
	u32 attachToID;
	xVec3 pos;
	xVec3 vel;
	f32 vel_angle_variation;
	type_17 rot;
	u8 padding;
	f32 radius;
	f32 emit_interval_current;
	void* emit_volume;
};

struct xCutsceneZbuffer
{
	f32 start;
	f32 end;
	f32 nearPlane;
	f32 farPlane;
};

struct xCamera : xBase
{
	RwCamera* lo_cam;
	xMat4x3 mat;
	xMat4x3 omat;
	xMat3x3 mbasis;
	xBound bound;
	xMat4x3* tgt_mat;
	xMat4x3* tgt_omat;
	xBound* tgt_bound;
	xVec3 focus;
	xScene* sc;
	xVec3 tran_accum;
	f32 fov;
	u32 flags;
	f32 tmr;
	f32 tm_acc;
	f32 tm_dec;
	f32 ltmr;
	f32 ltm_acc;
	f32 ltm_dec;
	f32 dmin;
	f32 dmax;
	f32 dcur;
	f32 dgoal;
	f32 hmin;
	f32 hmax;
	f32 hcur;
	f32 hgoal;
	f32 pmin;
	f32 pmax;
	f32 pcur;
	f32 pgoal;
	f32 depv;
	f32 hepv;
	f32 pepv;
	f32 orn_epv;
	f32 yaw_epv;
	f32 pitch_epv;
	f32 roll_epv;
	xQuat orn_cur;
	xQuat orn_goal;
	xQuat orn_diff;
	f32 yaw_cur;
	f32 yaw_goal;
	f32 pitch_cur;
	f32 pitch_goal;
	f32 roll_cur;
	f32 roll_goal;
	f32 dct;
	f32 dcd;
	f32 dccv;
	f32 dcsv;
	f32 hct;
	f32 hcd;
	f32 hccv;
	f32 hcsv;
	f32 pct;
	f32 pcd;
	f32 pccv;
	f32 pcsv;
	f32 orn_ct;
	f32 orn_cd;
	f32 orn_ccv;
	f32 orn_csv;
	f32 yaw_ct;
	f32 yaw_cd;
	f32 yaw_ccv;
	f32 yaw_csv;
	f32 pitch_ct;
	f32 pitch_cd;
	f32 pitch_ccv;
	f32 pitch_csv;
	f32 roll_ct;
	f32 roll_cd;
	f32 roll_ccv;
	f32 roll_csv;
	type_31 frustplane;
};

struct _anon1
{
};

struct xEntPenData
{
	xVec3 top;
	f32 w;
	xMat4x3 omat;
};

struct HAZRing : HAZTypical
{
	f32 hyt_ring;
};

type_133 buffer;
type_144 buffer;
type_95 g_JF01_cin_fodder;
type_141 g_RB01_cin_fodBomb;
type_217 g_RB01_cin_fodBzzt;
type_78 g_RB01_cin_chomper;
type_162 g_JF01_cin_hammer;
type_132 g_JF03_cin_tartar;
type_128 g_BB01_cin_glove;
type_63 g_GL01_cin_monsoon;
type_166 g_RB01_cin_sleepytime;
type_83 g_BC01_cin_arfarf;
type_57 g_BB01_cin_chuck;
type_198 g_KF01_cin_tubelet;
type_130 g_GY01_cin_slick;
type_34 g_GY01_cin_gy_open;
type_155 g_GY01_cin_gy_end;
type_20 g_B101_b1_open;
type_158 g_B101_b1_round2;
type_98 g_B101_b1_ending;
type_65 g_B201_b2_open;
type_189 g_B201_b2_round2;
type_92 g_B201_b2_round3;
type_202 g_B201_b2_ending;
type_97 g_B302_begin;
type_146 g_B302_end;
type_195 g_B303_end;
type_186 g_HB00_prologue;
type_142 g_cutmap;
NCINBeNosey* g_noz_ncin;
_anon1 __vt__11NCINBeNosey;
zGlobals globals;
zNPCB_SB2* _singleton;
xVec3 g_NX3;
xVec3 g_Z3;
xVec3 g_Y3;
xVec3 g_X3;
_anon0 __vt__9XCSNNosey;

void NCIN_SBBNode_AR(NCINEntry* fxrec, RpAtomic* model, RwMatrixTag* animMat, u32 animIndex);
void NCIN_SBBNode_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_BubbleTrail_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_BubbleTrail_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_EntityBonePar_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_GloveShrapnel_AR(NCINEntry* fxrec, RpAtomic* model, RwMatrixTag* animMat, u32 animIndex);
void NCIN_GloveShrapnel_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_SpatGlow_AR(NCINEntry* fxrec, RpAtomic* model, RwMatrixTag* animMat, u32 animIndex);
void NCIN_SpatGlow_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_PatBossShrapnel_AR(NCINEntry* fxrec, RpAtomic* model, RwMatrixTag* animMat, u32 animIndex);
void NCIN_PatBossShrapnel_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_GooLever_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_GooLever_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_B201HideBelt_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_FreezeBreath_AR(zCutsceneMgr* csnmgr, NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_FreezeBreath_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_B101Shockwave_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_LightninBone_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_LightninBone_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_Lightnin2Bones_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_Lightnin2Bones_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_HookRecoil_AR(zCutsceneMgr* csnmgr, RpAtomic* model, RwMatrixTag* animMat, u32 animIndex);
void NCIN_HookRecoil_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_BoneTrail_AR(RwMatrixTag* animMat, u32 animIndex);
void NCIN_BoneTrail_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_BombTrail_AR(RwMatrixTag* animMat, u32 animIndex);
void NCIN_BombTrail_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_MidFish_AR(RwMatrixTag* animMat, u32 animIndex);
void NCIN_MidFish_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_FodProdBone_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_FodProdBone_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_FodProd_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_FodProd_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_OilHazard(NCINEntry* fxrec, s32 killit);
void NCIN_ShieldPop(NCINEntry* fxrec, s32 killit);
void NCIN_FireSpiral_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_FireSpiral_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_PeteBonk(NCINEntry* fxrec, s32 killit);
void NCIN_MaryBoom(NCINEntry* fxrec, s32 killit);
void NCIN_SleepyDRay_AR(zCutsceneMgr* csnmgr, NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_SleepyDRay_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_SleepyLamp_AR(zCutsceneMgr* csnmgr, NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_SleepyLamp_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_ArfDogBoom(NCINEntry* fxrec, s32 killit);
void NCIN_TTGunSmoke_AR(zCutsceneMgr* csnmgr, NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_HazTTSteam_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_HazTTSteam_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_HazProjShoot(NCINEntry* fxrec, s32 killit);
void NCIN_WaterSplash(NCINEntry* fxrec, s32 killit);
void NCIN_HammerStreak_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_HammerStreak_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_HammerShock(NCINEntry* fxrec, s32 killit);
void NCIN_Zapper(NCINEntry* fxrec, s32 killit);
void NCIN_BubHit(NCINEntry* fxrec, s32 killit);
void NCIN_BubTrailBone_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex);
void NCIN_BubWipe(NCINEntry* fxrec, s32 killit);
void NCIN_BubSlam(NCINEntry* fxrec, s32 killit);
void NCIN_Generic_Upd(NCINEntry* fxrec, s32 killit);
void zNPCFXCutscene(zCutsceneMgr* csnmgr);
void zNPCFXCutsceneDone(zCutsceneMgr* csnmgr);
s32 zNPCFXCutscenePrep(zCutsceneMgr* csnmgr);
NCINEntry* zNPCFXCutscenePickTable(zCutsceneMgr* csnmgr);
void zNPCFXShutdown();
void zNPCFXStartup();
void UpdatedAnimated(NCINBeNosey* this, RpAtomic* model, RwMatrixTag* animMat, u32 animIndex, u32 dataIndex);
void CanRenderNow(NCINBeNosey* this);
void NCIN_Par_CIN_PLATFORM_JETS_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_Par_CIN_BIGDUP_SPAWN_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_Par_CIN_BIGDUP_SMOKE_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_Par_BPLANK_SBB_SMOKE_2_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_Par_BPLANK_SBB_SMOKE_1_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_Par_BPLANK_SBB_JET_2_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_Par_BPLANK_SBB_JET_1_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_Par_BPLANK_SBB_FLAMES_2_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_Par_BPLANK_SBB_FLAMES_1_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_Par_BPLANK_JET_2_Upd(NCINEntry* fxrec, s32 killit);
void NCIN_Par_BPLANK_JET_1_Upd(NCINEntry* fxrec, s32 killit);

// NCIN_SBBNode_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3ceb00
void NCIN_SBBNode_AR(NCINEntry* fxrec, RpAtomic* model, RwMatrixTag* animMat, u32 animIndex)
{
	NCINMat& data;
	s32 bones;
	RpGeometry* geom;
	RpSkin* skin;
	xMat3x3 imat;
	s32 i;
}

// NCIN_SBBNode_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3cecd0
void NCIN_SBBNode_Upd(NCINEntry* fxrec, s32 killit)
{
	NCINMat& data;
}

// NCIN_BubbleTrail_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3ced50
void NCIN_BubbleTrail_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	xVec3& offset;
	xVec3& root_vel;
	f32 rand_diameter;
	f32 rand_vel;
	f32 scale;
	NCINCustom& data;
	xVec3& oldloc;
	xVec3& oldvel;
	s32 total;
	xMat4x3 mat;
	xVec3 vel;
	xVec3 loc;
	xVec3 rloc;
	xVec3 rvel;
}

// NCIN_BubbleTrail_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3cf0b0
void NCIN_BubbleTrail_Upd(NCINEntry* fxrec, s32 killit)
{
	NCINCustom& data;
}

// NCIN_EntityBonePar_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3cf0f0
void NCIN_EntityBonePar_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	f32 dt;
	xParEmitter* pe;
	xParEmitterAsset* a;
	xParEmitterPropsAsset* prop;
	xParSys* ps;
	xParGroup* g;
	s32 rate_has_elapsed;
	f32 rate;
	s32 count;
	f32 life;
	f32 size_birth;
	f32 size_death;
	xMat4x3 bone_mat;
	xVec3 oldloc;
	xVec3 loc;
	xVec3 vel;
	s32 i;
	xPar* p;
	s32 c;
	f32 fc1;
	f32 fc2;
}

// NCIN_GloveShrapnel_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3cf6e0
void NCIN_GloveShrapnel_AR(NCINEntry* fxrec, RpAtomic* model, RwMatrixTag* animMat, u32 animIndex)
{
}

// NCIN_GloveShrapnel_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3cf780
void NCIN_GloveShrapnel_Upd(NCINEntry* fxrec, s32 killit)
{
	zShrapnelAsset* sasset;
}

// NCIN_SpatGlow_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3cf810
void NCIN_SpatGlow_AR(NCINEntry* fxrec, RpAtomic* model, RwMatrixTag* animMat, u32 animIndex)
{
	s32 boneIndex;
	iColor_tag color;
}

// NCIN_SpatGlow_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3cf9c0
void NCIN_SpatGlow_Upd(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_PatBossShrapnel_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3cf9e0
void NCIN_PatBossShrapnel_AR(NCINEntry* fxrec, RpAtomic* model, RwMatrixTag* animMat, u32 animIndex)
{
}

// NCIN_PatBossShrapnel_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3cfa30
void NCIN_PatBossShrapnel_Upd(NCINEntry* fxrec, s32 killit)
{
	zShrapnelAsset* sasset;
}

// NCIN_GooLever_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3cfac0
void NCIN_GooLever_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	f32 fudgeRate;
}

// NCIN_GooLever_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3cfb40
void NCIN_GooLever_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter* emitter;
}

// NCIN_B201HideBelt_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3cfbd0
void NCIN_B201HideBelt_Upd(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_FreezeBreath_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3cfd00
void NCIN_FreezeBreath_AR(zCutsceneMgr* csnmgr, NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	xVec3 pnt1;
	xVec3 pnt2;
	s32 boneIndex;
}

// NCIN_FreezeBreath_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3cfee0
void NCIN_FreezeBreath_Upd(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_B101Shockwave_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3cff40
void NCIN_B101Shockwave_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter* emitter;
}

// NCIN_LightninBone_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d0070
void NCIN_LightninBone_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	xVec3 pnt1;
	xVec3 pnt2;
	s32 boneIndex;
}

// NCIN_LightninBone_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d0270
void NCIN_LightninBone_Upd(NCINEntry* fxrec, s32 killit)
{
	_tagLightningAdd addInfo;
	xVec3 pnt;
	zLightning* light;
}

// NCIN_Lightnin2Bones_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d0440
void NCIN_Lightnin2Bones_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	zLightning* light;
	xVec3 pnt1;
	s32 boneIndex;
	xVec3 pnt1;
	s32 boneIndex;
}

// NCIN_Lightnin2Bones_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d06c0
void NCIN_Lightnin2Bones_Upd(NCINEntry* fxrec, s32 killit)
{
	_tagLightningAdd addInfo;
	xVec3 pnt;
}

// NCIN_HookRecoil_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d0770
void NCIN_HookRecoil_AR(zCutsceneMgr* csnmgr, RpAtomic* model, RwMatrixTag* animMat, u32 animIndex)
{
	s32 idx_anim;
	u32 num_bones;
	xMat4x3* mat_root;
	u32 i;
	xVec3 pos_emit;
}

// NCIN_HookRecoil_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d08c0
void NCIN_HookRecoil_Upd(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_BoneTrail_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d08e0
void NCIN_BoneTrail_AR(RwMatrixTag* animMat, u32 animIndex)
{
}

// NCIN_BoneTrail_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d0910
void NCIN_BoneTrail_Upd(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_BombTrail_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d0930
void NCIN_BombTrail_AR(RwMatrixTag* animMat, u32 animIndex)
{
}

// NCIN_BombTrail_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d0960
void NCIN_BombTrail_Upd(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_MidFish_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d0980
void NCIN_MidFish_AR(RwMatrixTag* animMat, u32 animIndex)
{
	xMat4x3* mat_root;
	s32 i;
	xVec3 pos_emit;
	xMat4x3* mat_root;
	s32 idx_bone;
	xVec3 pos_emit;
	type_108 g_idx_handbone;
}

// NCIN_MidFish_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d0b20
void NCIN_MidFish_Upd(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_FodProdBone_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d0b40
void NCIN_FodProdBone_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	xMat4x3* mat_root;
	xMat4x3* mat_bone;
	NPCHazard* haz;
	xVec3 pos_poke;
	xVec3 vec_offset;
	s8 @7361;
	s32 idx_bonePoke;
	s8 @7353;
}

// NCIN_FodProdBone_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d0e20
void NCIN_FodProdBone_Upd(NCINEntry* fxrec, s32 killit)
{
	NPCHazard* haz;
	NPCHazard* haz;
	s32 rc;
	f32 tym;
}

// NCIN_FodProd_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d0f40
void NCIN_FodProd_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	xMat4x3* mat_root;
	xMat4x3* mat_bone;
	NPCHazard* haz;
	xVec3 pos_poke;
	xVec3 vec_offset;
}

// NCIN_FodProd_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d1170
void NCIN_FodProd_Upd(NCINEntry* fxrec, s32 killit)
{
	NPCHazard* haz;
	NPCHazard* haz;
	s32 rc;
	f32 tym;
}

// NCIN_OilHazard__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d1290
void NCIN_OilHazard(NCINEntry* fxrec, s32 killit)
{
	NPCHazard* haz;
	xVec3 pos_emit;
	s32 i;
	xVec3 vel_emit;
	NPCHazard* haz;
	s32 rc;
	f32 tym;
}

// NCIN_ShieldPop__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d1790
void NCIN_ShieldPop(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_FireSpiral_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d17f0
void NCIN_FireSpiral_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	xMat4x3* mat_root;
	xVec3 pos_emit;
	xVec3 dir_emit;
	xVec3 vel;
	f32 tym;
}

// NCIN_FireSpiral_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d19d0
void NCIN_FireSpiral_Upd(NCINEntry* fxrec, s32 killit)
{
	NPARMgmt* mgr;
}

// NCIN_PeteBonk__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d1a10
void NCIN_PeteBonk(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_MaryBoom__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d1a70
void NCIN_MaryBoom(NCINEntry* fxrec, s32 killit)
{
	NPCHazard* haz;
	NPCHazard* haz;
	s32 rc;
	f32 tym;
}

// NCIN_SleepyDRay_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d1b40
void NCIN_SleepyDRay_AR(zCutsceneMgr* csnmgr, NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	xMat4x3* mat_root;
	xVec3* pos_orig;
	s32 i;
	f32 rat;
	xVec3 pos_emit;
	xMat4x3* mat_root;
	xVec3 pos_dray;
	RwRGBA rgba_top;
	RwRGBA rgba_bot;
	f32 tym_instate;
	NPCCone conedata;
	type_160 uv_slice_dray;
	type_149 uv_scroll_dray;
}

// NCIN_SleepyDRay_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d1d90
void NCIN_SleepyDRay_Upd(NCINEntry* fxrec, s32 killit)
{
	s8* nam_texture;
}

// NCIN_SleepyLamp_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d1e10
void NCIN_SleepyLamp_AR(zCutsceneMgr* csnmgr, NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	NCINLamp* lampdat;
	xMat4x3* mat_root;
	NCINLamp* lampdat;
	xVec3 pos_lamp;
	RwRGBA rgba_top;
	RwRGBA rgba_bot;
	xVec3 rgb_peace;
	xVec3 rgb_anger;
	f32 rat;
	NPCCone conedata;
	type_181 tym_anger;
}

// NCIN_SleepyLamp_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d2230
void NCIN_SleepyLamp_Upd(NCINEntry* fxrec, s32 killit)
{
	s8* nam_texture;
}

// NCIN_ArfDogBoom__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d2290
void NCIN_ArfDogBoom(NCINEntry* fxrec, s32 killit)
{
	NPCHazard* haz;
	NPCHazard* haz;
	s32 rc;
	f32 tym;
}

// NCIN_TTGunSmoke_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d2370
void NCIN_TTGunSmoke_AR(zCutsceneMgr* csnmgr, NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	u32 idx_roboAnim;
	s32 idx_boneGun;
	xMat4x3* mat_root;
	xMat4x3* mat_bone;
	xVec3 vec_offset;
	xVec3 pos_smoke;
	f32 spd_blow;
	type_6 tym_blow;
	xCutscene* csn;
	f32 blowPeriod;
	f32 tym_blown;
	f32 rat_blow;
	f32 bowl;
	xVec3 dir_blow;
	xVec3 vel_smoke;
}

// NCIN_HazTTSteam_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d2910
void NCIN_HazTTSteam_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	s32 idx_boneSign;
	xVec3 vec_offset;
	xMat4x3* mat_root;
	xMat4x3* mat_bone;
	NPCHazard* haz;
}

// NCIN_HazTTSteam_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d2c90
void NCIN_HazTTSteam_Upd(NCINEntry* fxrec, s32 killit)
{
	NPCHazard* haz;
	NPCHazard* haz;
	s32 rc;
	f32 tym;
}

// NCIN_HazProjShoot__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d2d90
void NCIN_HazProjShoot(NCINEntry* fxrec, s32 killit)
{
	NPCHazard* haz;
	en_npchaz use_haztyp;
	NPCHazard* haz;
	s32 rc;
	HAZTarTar* tartar;
	xVec3 pos_dest;
	xVec3 diff;
	f32 tym;
	NPCHazard* haz;
}

// NCIN_WaterSplash__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d2f90
void NCIN_WaterSplash(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_HammerStreak_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d3000
void NCIN_HammerStreak_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	type_208 idx_hamBone;
	xMat4x3* mat_root;
	type_5 mat_hamBone;
	xVec3 left;
	xVec3 right;
	xVec3 top;
	xVec3 bottom;
	NCINStrk* strkdat;
}

// NCIN_HammerStreak_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d3240
void NCIN_HammerStreak_Upd(NCINEntry* fxrec, s32 killit)
{
	NCINStrk* strkdat;
	NCINStrk* strkdat;
}

// NCIN_HammerShock__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d32c0
void NCIN_HammerShock(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_Zapper__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d3320
void NCIN_Zapper(NCINEntry* fxrec, s32 killit)
{
	NCINData* fxdat;
	_tagLightningAdd info;
}

// NCIN_BubHit__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d3470
void NCIN_BubHit(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_BubTrailBone_AR__FPC12zCutsceneMgrP9NCINEntryP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d34d0
void NCIN_BubTrailBone_AR(NCINEntry* fxrec, RwMatrixTag* animMat, u32 animIndex)
{
	s32 idx_boneInterest;
	xVec3 pos_emit;
	xMat4x3* mat_bone;
}

// NCIN_BubWipe__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d3570
void NCIN_BubWipe(NCINEntry* fxrec, s32 killit)
{
	xMat4x3* mat;
	xVec3* pos;
	xVec3* vel;
	xVec3 vec_infront;
	xVec3* pp;
	xVec3* vp;
	s32 i;
	xVec3 vel_wall;
	xVec3 scl_wall;
	xMat4x3 mat_fake;
}

// NCIN_BubSlam__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d3880
void NCIN_BubSlam(NCINEntry* fxrec, s32 killit)
{
}

// NCIN_Generic_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d3940
void NCIN_Generic_Upd(NCINEntry* fxrec, s32 killit)
{
}

// zNPCFXCutscene__FPC6xScenefPC12zCutsceneMgr
// Start address: 0x3d3960
void zNPCFXCutscene(zCutsceneMgr* csnmgr)
{
	NCINEntry* fxtab;
	xCutscene* csn;
	s32 need_animated;
	s32 need_render;
	NCINEntry* nextrec;
	NCINEntry* fxrec;
	s32 flags;
}

// zNPCFXCutsceneDone__FPC6xScenefPC12zCutsceneMgr
// Start address: 0x3d3b30
void zNPCFXCutsceneDone(zCutsceneMgr* csnmgr)
{
	NCINEntry* fxtab;
	NCINEntry* nextrec;
	NCINEntry* fxrec;
}

// zNPCFXCutscenePrep__FPC6xScenefPC12zCutsceneMgr
// Start address: 0x3d3c10
s32 zNPCFXCutscenePrep(zCutsceneMgr* csnmgr)
{
	NCINEntry* fxtab;
	NCINEntry* nextrec;
	NCINEntry* fxrec;
	NCINEntry* fxrec;
	type_45 tweakBase;
	type_53 tweakName;
}

// zNPCFXCutscenePickTable__FPC12zCutsceneMgr
// Start address: 0x3d3d80
NCINEntry* zNPCFXCutscenePickTable(zCutsceneMgr* csnmgr)
{
	NCINEntry* da_fxtab;
	NCINMap* cutrec;
	s32 needinit;
}

// zNPCFXShutdown__Fv
// Start address: 0x3d3e50
void zNPCFXShutdown()
{
}

// zNPCFXStartup__Fv
// Start address: 0x3d3e60
void zNPCFXStartup()
{
	NCINBeNosey nozey_npc_cinematics;
	s8 @6424;
}

// UpdatedAnimated__11NCINBeNoseyFP8RpAtomicP11RwMatrixTagUiUi
// Start address: 0x3d3e90
void UpdatedAnimated(NCINBeNosey* this, RpAtomic* model, RwMatrixTag* animMat, u32 animIndex, u32 dataIndex)
{
	zCutsceneMgr* csnmgr;
	NCINEntry* fxtab;
	NCINEntry* nextrec;
	NCINEntry* fxrec;
}

// CanRenderNow__11NCINBeNoseyFv
// Start address: 0x3d3f50
void CanRenderNow(NCINBeNosey* this)
{
	zCutsceneMgr* csnmgr;
	NCINEntry* fxtab;
	NCINEntry* nextrec;
	NCINEntry* fxrec;
}

// NCIN_Par_CIN_PLATFORM_JETS_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d3fd0
void NCIN_Par_CIN_PLATFORM_JETS_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter&* emitter;
	xParEmitterAsset* a;
}

// NCIN_Par_CIN_BIGDUP_SPAWN_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d40f0
void NCIN_Par_CIN_BIGDUP_SPAWN_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter&* emitter;
	xParEmitterAsset* a;
}

// NCIN_Par_CIN_BIGDUP_SMOKE_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d4210
void NCIN_Par_CIN_BIGDUP_SMOKE_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter&* emitter;
	xParEmitterAsset* a;
}

// NCIN_Par_BPLANK_SBB_SMOKE_2_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d4330
void NCIN_Par_BPLANK_SBB_SMOKE_2_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter&* emitter;
	xParEmitterAsset* a;
}

// NCIN_Par_BPLANK_SBB_SMOKE_1_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d4450
void NCIN_Par_BPLANK_SBB_SMOKE_1_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter&* emitter;
	xParEmitterAsset* a;
}

// NCIN_Par_BPLANK_SBB_JET_2_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d4570
void NCIN_Par_BPLANK_SBB_JET_2_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter&* emitter;
	xParEmitterAsset* a;
}

// NCIN_Par_BPLANK_SBB_JET_1_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d4690
void NCIN_Par_BPLANK_SBB_JET_1_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter&* emitter;
	xParEmitterAsset* a;
}

// NCIN_Par_BPLANK_SBB_FLAMES_2_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d47b0
void NCIN_Par_BPLANK_SBB_FLAMES_2_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter&* emitter;
	xParEmitterAsset* a;
}

// NCIN_Par_BPLANK_SBB_FLAMES_1_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d48d0
void NCIN_Par_BPLANK_SBB_FLAMES_1_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter&* emitter;
	xParEmitterAsset* a;
}

// NCIN_Par_BPLANK_JET_2_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d49f0
void NCIN_Par_BPLANK_JET_2_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter&* emitter;
	xParEmitterAsset* a;
}

// NCIN_Par_BPLANK_JET_1_Upd__FPC12zCutsceneMgrP9NCINEntryi
// Start address: 0x3d4b10
void NCIN_Par_BPLANK_JET_1_Upd(NCINEntry* fxrec, s32 killit)
{
	zParEmitter&* emitter;
	xParEmitterAsset* a;
}

